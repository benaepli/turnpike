...
- kind: add | category: scheduler | origin: proposer | status: KEPT at iteration 34 (judge gain 4, cost 0, rank 2) | parent: iteration-34 feedback round
...
- kind: add | category: scheduler | origin: proposer | status: ADMITTED at iteration 38 (judge gain 6, cost 0; the first opposite-side arm on the crash-timing axis; expected to lose on the VR primary and to be kept only if the named panel members read up per cell on the candidate binary) | parent: crash placement and crash phase (merged), the per-cell panel read
...
- kind: add | category: scheduler | origin: proposer | status: KEPT at iteration 39 (judge gain 2, cost 2, rank 4) | parent: iteration-39 delivery-axis round
