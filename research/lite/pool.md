...
- kind: add | category: scheduler | origin: operator-agent | status:
  KEPT at iteration 40 (judge gain 5, cost 0, rank 4; prediction rewritten: Kept at rank 4 rather than closed because the census it is gated behind is cheap, arm-blind, and worth having on its own: nobody has measured what the recovered node's acted-entry count actually is when the chain's DoViewChange lands. If the census says the distribution is tight, the arm becomes a s) | parent:
...
- kind: add | category: scheduler | origin: proposer | status: FILED at iteration 42 as built, and RE-ADMITTED as iteration 43 in the admitted layer order. The plan placed the preference AFTER fresh-first and pair order; the build placed it BEFORE them, so it displaced fresh-first's own choice in 148,549,753 of 149,998,186 swaps. Fresh-first is worth a factor of four at depth 8 (iteration 39), so the session measured origin stickiness overriding fresh-first, not origin stickiness as a tiebreak: depth>=8 0.749 [0.729, 0.770] while depth>=6 and 7 lost only 5.5 percent. The mechanism as admitted has not been graded | parent: iteration-40 dispatch-draw round
...
- kind: add | category: scheduler | origin: proposer | status: KEPT at iteration 40 (judge gain 6, cost 0, rank 3) | parent: iteration-40 dispatch-draw round
...
- kind: add | category: scheduler | origin: operator-agent | status:
  KEPT at iteration 42 (judge gain 5, cost 0, rank 3; two claims in the seeded form were false - the send ledger carries counts, not destinations (they are reachable by scanning the network queue and purgatory at the arming site, which is cheaper than a per-destination ledger array), and the predicate as written cannot fire on the chain because node 2 crashes with its StartViewChange to node 0 undelivered and node 0 never restarts; the supported form is EARLY and at least one in-flight record addressed to a restarted node, bit 16384, band [0.95, 1.10], decisive on depth>=8 itself, and the arm SUBSTITUTES into a same-length treated table so the draw modulus stays 3) | parent:
