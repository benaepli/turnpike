//! How a generated plan orders its node restarts relative to client work.
//!
...
//! the step budget ends it. Each run takes one of three cells: stock keeps
//! the generator as it is, exempt never lets a restart wait on a
//! probabilistic edge, and forced gives every restart one client request
//! to wait on, the inverse of exempt.
//!
...
pub enum RecoverDeps {
    ...
    Exempt,
    /// After the pass, every restart takes one extra edge from a client
    /// request that was not already ordered relative to it.
    Forced,
}

impl RecoverDeps {
    /// The cell the workload seed names: one quarter exempt, one quarter
    /// forced, and the remaining half stock.
    pub fn of_workload_seed(workload_seed: u64) -> Self {
        match run_phase::salted_phase(workload_seed as i64, DEPS_SALT, 4) {
            0 => RecoverDeps::Exempt,
            1 => RecoverDeps::Forced,
            _ => RecoverDeps::Stock,
        }
    }
...
mod tests {
    ...
    fn the_cell_is_a_function_of_the_workload_seed_alone() {
        ...
        assert_eq!(RecoverDeps::Forced.index(), 2);
    }

    #[test]
    fn each_cell_gets_close_to_its_share_over_derived_seeds() {
        let n = 40_000i64;
        let mut counts = [0u32; 3];
        for run_id in 0..n {
            let seed = derive_seed(7, run_id, WORKLOAD_SALT);
            counts[RecoverDeps::of_workload_seed(seed).index()] += 1;
        }
        let share = |i: usize| counts[i] as f64 / n as f64;
        assert!((share(0) - 0.5).abs() < 0.02, "stock takes {}", share(0));
        assert!((share(1) - 0.25).abs() < 0.02, "exempt takes {}", share(1));
        assert!((share(2) - 0.25).abs() < 0.02, "forced takes {}", share(2));
    }
    ...
    fn the_cell_follows_the_seed_and_not_the_id_that_issued_the_run() {
        ...
        for run_id in 0..n {
            ...
            let by_id = match run_phase::salted_phase(run_id, DEPS_SALT, 4) {
                0 => RecoverDeps::Exempt,
                1 => RecoverDeps::Forced,
                _ => RecoverDeps::Stock,
            };
        ...
        // Two independent draws over {1/4, 1/4, 1/2} agree with probability
        // 1/16 + 1/16 + 1/4 = 3/8.
        let share = agree as f64 / n as f64;
        assert!((share - 0.375).abs() < 0.02, "the seed's cell follows the id on {share}");
    }
}
