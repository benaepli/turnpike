...
out = root / 'tmp/loop/lite/eval-transient'
...
for b in base['chunks']:
    seed = b['seed']
    path = root / f'research/lite/state/eval-transient/chunk-{seed}.cand.json'
    if not path.exists():
        continue
    ...
    for ba, ca in zip(bm['campaign']['arms'], cm['campaign']['arms']):
        ...
        arms.append(dict(arm=ba['id'], base=aggregate([ba]), candidate=aggregate([ca]),
                         throughput_ratio=(ca['gradedRuns']/ca['wallMs'])/(ba['gradedRuns']/ba['wallMs']),
    ...
    support = {k:v for k,v in c['utilStats']['counters'].items() if k.startswith('eval_transient.')}
