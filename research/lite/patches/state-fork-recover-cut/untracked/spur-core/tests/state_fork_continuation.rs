//! A state-fork child continues its parent's run from the snapshot taken
//! when the node whose queued crash armed the fault-crossing signal applies
//! its planned recover. Drawing from a copy of the parent's generators at
//! the snapshot, it has to reproduce the parent's whole event sequence and
//! ending; drawing from its own generators, it has to agree with the parent
//! through the recover step. Its row carries the steps of prefix and
...
fn check_fork() {
    ...
    for (i, &parent_id) in parents.iter().enumerate() {
        ...
        let Some(fork) = parent.fork.as_deref() else {
            // The signal fired but the run ended before the recover the
            // snapshot waits for.
            no_cut += 1;
            let _ = fs::remove_dir_all(&parent_dir);
            continue;
        ...
        let cut_node = fork
            ...
            .state
            .replay_cut_node
            .expect("a parent that fired the signal names the node it waits for");
        let recovers: Vec<i32> = fork
            .path_state
            .history
            .iter()
            .filter(|op| {
                op.op_action == "System.Recover"
                    && op.step > cut.step
                    && op.payload[0].as_node().expect("a recover names its node").index
                        == cut_node
            })
            .map(|op| op.step)
            .collect();
        assert_eq!(
            recovers,
            vec![fork.locals.steps() - 1],
            "parent {parent_id}: the snapshot does not stand on the step after the first \
             recover of node {cut_node} past the signal at step {}",
            cut.step
        );
        ...
        // through the recover step and is free after it.
        let fork_step = fork.locals.steps() - 1;
        ...
        assert_eq!(
            events_through(&own_dir, fork_step),
            events_through(&parent_dir, fork_step),
            "parent {parent_id}: the child's events up to step {fork_step} differ"
        );
    ...
    assert!(
        checked > 0,
        "no candidate parent took a snapshot ({no_cut} fired the signal without one)"
    );
}
