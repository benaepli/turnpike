...
state = json.loads((root / 'research/lite/state/recover-deps-merge.json').read_text())
...
def gates(c):
    ...
    out = dict(
        ...
        forced_edges=None, forced_floor_30k=None,
        ...
        forced_over_stock_unrec_pos=None,
...
for bc in base['chunks']:
    path = root / f"research/lite/state/recover-deps-merge/chunk-{bc['seed']}.cand.json"
    if not path.exists():
        continue
