...
out = root / 'tmp/loop/lite/replay-frontier'
...
for b in base['chunks']:
    seed = b['seed']
    path = root / f'research/lite/state/replay-frontier/chunk-{seed}.cand.json'
    if not path.exists():
        continue
    ...
    if raw.exists():
        p = json.loads(raw.read_text())['replay_frontier']
        support = {k:v for k,v in p.items() if k not in ['by_arm','parent_diagnostics']}
        def fraction(a,b):
            return p[a]/p[b] if p[b] else None
        support.update(applied_share=fraction('applied_runs','assigned_requests'),
                       usable_suffix_share=fraction('usable_suffix_runs','applied_runs'),
                       divergent_share=fraction('divergent_runs','applied_runs'),
                       repeated_applied_pair_share=fraction('applied_pair_repeats','applied_runs'),
                       retained_tape_ratio=fraction('cumulative_retained_words','cumulative_original_cut_words'))
