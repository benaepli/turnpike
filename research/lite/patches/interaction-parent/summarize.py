...
out = root / 'tmp/loop/lite/interaction-parent'
...
for b in base['chunks']:
    seed = b['seed']
    path = root / f'research/lite/state/interaction-parent/chunk-{seed}.cand.json'
    if not path.exists():
        continue
    ...
    if raw.exists():
        p = json.loads(raw.read_text())['arm_selector_axis']['interaction_parent']
        ...
        support.update(qualified_cells=sum(x['supported_decisions']>=100 for x in p['cells']),
                       ...
                       supported_actual_change_share=p['supported_changed_child_choices']/p['supported_decisions'],
                       supported_parent_departure_share=p['supported_parent_departures']/p['supported_learner_draws'])
