...
The observable inverts as required and the dose separates cleanly. Mean
same-sender streak at a destination: control 1.984, full cell 1.460 (0.736
of control, inside the 0.80 bar), quarter cell 1.704 (0.859, the expected
weaker dose). Swaps 1,409,162 on the full cell and 128,200 on the quarter;
the override gate reads exactly zero in both cells, so the arm never
displaces a merged preference. Contests partition exactly in both cells.
Two naming notes from the implementer: the decline counter the plan called
same_origin_already is exported as drawn_already_differs, since under
alternation it counts a pick whose sender already differs, and a fifth
