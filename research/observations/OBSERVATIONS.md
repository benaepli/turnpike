...
## 2026-08-29T11:40:57.504Z

**act-conditioned-perturbation** (closed): Gating bias/delay/restart selection on a pre-delivery act-relevance predicate (exec.rs/scheduler.rs/state.rs + util_stats.rs, no new knob: 18 params before/after) did move the instrumented target but not the ladder. Biased acted-fraction 15.2%->20.5% (1790539/8757475), delayed 15.5%->21.4% (1748593/8158094), senderRestarted 14.9%->16.3%, receiverRestarted flat at 2.3% (10756/470318) — i.e. ~5pp of the promised ~38pp gap, nowhere near the 53% all-deliveries reference. Meanwhile the reference itself collapsed: all.acted 53% -> 37.1% (61830317/166693472), so the gate changed the delivery population/execution shape, not just the targeting. Ladder: rejected after 1 chunk / 261420 runs, depth>=4 per-run ratio 0.98, depth>=7 -1.6%; only depth>=5/6 nominally up (+1.7%/+2.3%) on +2.8% throughput (869 runs/s), 0 violations, max depth 9 (2 runs at >=8). Mechanistic reading: 'receiver acts on this delivery now' is not the right eligibility test, because (a) it is evaluated at selection time while the actual delivery happens after other events mutate the receiver, and (b) the counterfactual value of delaying/biasing a currently-inert delivery is precisely that it becomes act-relevant later — filtering those out removes the interleavings the ladder rewards. The deliveryEffects 'acted' counter is a post-hoc effect measure, not a validated targeting signal; treat the original 85%-waste framing as falsified. Side observations from the same run: steerAuthority preferenceExpressed 150976 over 767560757 steps (0.02% of steps) with only 29333 honored (19.4%), and termination is still 71.5% iterationsExhausted (187001) vs 28.4% planComplete at 74.7% step-budget utilisation.

## 2026-08-29T12:01:56.000Z

**stall-abort-progress-termination** (closed): Rejected on 1 chunk / 464,940 runs: every objective regressed per run (depth>=4 0.81, >=6 0.70, >=7 0.69, throughput 0.83, 0 violations both sides). Two distinct failures, and the second is the more useful datum. (1) The mechanism appears never to have fired: the util_stats termination block reports only runs/planComplete/iterationsExhausted(71,194)/deadlock(178)/stepsUsedSum — no stall_aborts or steps_reclaimed key, and stepsUsedSum/stepBudgetSum = 437.5M/2052.5M = 21%, i.e. no budget was reclaimed. Cause is almost certainly the threshold rule: 'longest quiet stretch observed in any completed run' is a max, so it saturates on the first completed-but-idle-tailed run and thereafter sits near the full step budget, monotonically un-firing forever. A max over a heavy-tailed distribution is not a session-derived threshold, it is an upper bound. Any future session-derived gate must use a quantile (p90/p95) and be allowed to move down. (2) Depth per run fell 20-30% even though nothing fired, which a pure no-op cannot do. The diff touched core/state.rs, coverage.rs, curriculum.rs and explorer.rs to add node_state_token; adding a token to State most plausibly perturbed state identity/coverage keys and therefore the exploration frontier itself. Lesson: instrumentation placed inside the state-identity path is not observation-only, it is an intervention. Throughput 0.83 additionally prices the per-step bookkeeping at ~17% of runs/s, which alone would need >20% depth-per-run gain to break even. Orthogonal but strong signal from the campaign arms: grid-short (max_iterations 1500) achieved rewardRate 685/s against 320-363/s for the four full-budget arms, ~2x, with depth>=6 6,660/163,800 = 0.041 vs grid's 3,006/76,140 = 0.039 — i.e. cutting the step budget roughly doubled reward throughput at equal per-run depth rate. That is the same thesis as this hypothesis (stopped runs waste budget) delivered by a knob that costs nothing in the hot loop, and it is the direction worth pursuing.

## 2026-08-29T12:40:24.690Z

**adaptive-step-budget-completion-quantile** (closed): Rejected on the first sequential chunk (456,060 runs, seed 1000): throughput ratio 0.796 and depth ratios 0.68 (>=6) / 0.77 (>=4) — the treatment was both slower AND shallower than baseline. Mechanism check says the feature worked as designed: stepBudgetSum/runs fell from 6000 to ~1820 (p95 converged near grid-short's 1500) and stepsUsed/stepBudget rose from 21% to 81%, so budget_capped clearly fired on most runs. The predicted win still did not materialize, which falsifies the parent's seeding evidence rather than the implementation. Two distinct lessons: (1) The campaign 'termination_completed' rewardRate is length-confounded — a shorter budget mechanically ends more runs per second, so grid-short's 685/s (528/s here) measures run turnover, not ladder progress. Per-run depth>=6 in this session fell ~15% (0.0349 vs ~0.0409 implied baseline) while total runs fell too; truncating at the completion p95 discards exactly the tail runs that reach depth 6-9 (only 5 runs reached depth 9, 504 reached depth 8 — these live far past p95 by construction). A quantile cap is self-defeating for a tail-seeking objective: it is defined to cut the tail. (2) Throughput dropped 20% even though mean steps/run fell ~4x. Shorter runs cannot be slower per step, so the loss is per-run fixed cost — the shared feedback-store p95 read/update on every run start and completion across 30 rayon threads. That cost (~20%) is larger than anything this family of hypotheses can pay back, and it taxes every future feedback-store-mediated mechanism, not just this one. Also note iterationsExhausted stayed dominant at 73% of runs (334,524), so capping did not convert exhausted runs into completions — it converted long exhausted runs into short exhausted runs. No violations found in either arm; h1/h2/h3 deltas were ~0.

## 2026-08-29T13:37:13.684Z

**steer-path-fixed-overhead-ablation** (auto_merge): The bisection landed as instrumentation, not as the promised per-stage throughput measurement. Three independent hunks in spur-core/src/simulator/core/scheduler.rs now skip work whenever ResolvedTerms::any_predicate() is false, each counted separately under utilization.json -> steer_empty_slice: (1) candidate mask -- per-candidate State::term_mask reads in select_within_queue; (2) ranking pass -- a second full pass over the eligible set, one runnable_novelty per candidate; (3) queue audit -- audit_steer_preference in schedule_runnable, one runnable_novelty + one term_mask per runnable in every queue at every scheduling point, plus steer_authority atomics. 25 s zero-weight campaign counts: 65,490,276 candidate-mask skips, 29,252,733 ranking-pass skips (ranking only fires when >1 candidate eligible, 45% of selections), 65,495,529 queue-audit skips over 21,480 runs. Since the audit is per-runnable rather than per-selection, its true work dominates the equal-looking candidate-mask count. Gating discovery that the plan did not anticipate: the queue-audit stage exists only when feedback.steer_audit is on and the other two only when stats is on, so a large slice of the 27-point residual is reachable by config alone, no rebuild. What was NOT obtained: the validation runs. Both sequential evals ran the 5-arm general_vr campaign with steering entirely off (steerAuthority.steps = 0, 866/864 runs/s, seeds 1000/1001), so nothing was compared against arm-000 (389) or the no-steer baseline (575); no stage was shown to recover >=15%, and bit-identity at a nonzero arm (339,717 expressed / 119,762 honored) was never re-checked. The auto_merge is 'non-inferior with the flag off' -- it proves the hunks cost nothing when unused, not that they buy anything. Objectives moved within noise (throughput +0.024, primary/depth>=6 +0.042, violations 0), params 18->19. Net: the residual is now named and instrumented but still unattributed in runs/s, and the dominant suspect (per-runnable audit work) is also paid at nonzero weights, i.e. it is a real steer-campaign cost, not only a zero-weight artifact.

## 2026-08-29T13:42:53.860Z

### Audit @5320
Evaluate dominates at 1815 s/iter = 53.3% of the 3403 s mean iteration, but only ~300-1200 s of that is actual exposure: the latest merged hypothesis carried exposure=301 s (one chunk) against an evaluate phase of ~512 s in iter 5319, i.e. ~40% of evaluate wall is non-exploring overhead; iter 5306 spent 4320 s of evaluate for at most 1200 s of 4-chunk exposure (72% overhead). Regression adds another 544 s (16.0%) and rejudge 201 s (5.9%) — 22% of every iteration is spent re-confirming rather than exploring. Implement is 767 s (22.5%), of which 383 s (50%) is model/think and only ~106 s is build+smoke+test; 14.3 edits and 3.9 cargo builds per iteration. Propose is effectively free (0.26 ms/iter), which is exactly why the pool has inflated to 175 with 68 parked + 8 blocked + 7-11 proposed = 46-50% dead or dormant inventory that no phase is charged for maintaining. Net: ~78% of wall goes to measuring, ~22% to changing, and the measurement is under-powered (see below) — the worst possible split.

Goodhart: Objective is rung events per explore-SECOND, so truncation buys score: grid-short (max_iterations=1500) delivers 1995 runs/s and 78.9 depth>=6/s vs full grid's 598 runs/s and 22.9 — 3.4x the objective purely from shorter runs. The ladder is the union of arms, so the campaign score is largely a report on how much of the budget lands in the truncated arm.; runsPerSec is 860.8 vs reference 142.4 (6.0x) while meanPrefixDepth drifted 2.26 -> 3.28 and P(depth>=4) 0.034 -> 0.467 (13.7x). The sampled distribution no longer resembles the reference it is scored against; 'more rung events/s' is being bought by sampling a different, deeper, cheaper population rather than by better search.; h4Rate = 0.915 against a reference of 0. A predicate that fires on 91.5% of runs and 0% of reference runs is either trivially satisfiable or mis-specified; it is currently ladder decoration that can only move down, and it inflates the appearance of a rich metric ladder.; Campaign reward is termination_completed while allocation is round_robin — the reward is computed (grid-short 500.5 vs grid 164.6) but never used to allocate. It is a score that looks like optimization pressure and applies none; if it is ever switched on it will hard-select the truncated arm.; Nine of 29 merges' worth of recent effort targets the steer path (steer-path-fixed-overhead-ablation merged, steer-term-zero-weight-shortcircuit, steer-term-factorial-campaign, steer-term-base-rates, steer-term-reach-and-honor-diagnostic) while steer.evaluations = 0 and steer_authority.steps = 0. Measured 'wins' on a code path that makes zero decisions are pure measurement-noise harvesting.; The two bookkeeping counters disagree at the same iteration (pool table: parked 68 / proposed 7; time ledger: parked 66 / proposed 11), so backlog health is not being tracked reliably.

Utilization: steer / steer_authority=unexercised, multiplier_authority=broken, aos=unrewarding, dedup=unexercised, curriculum=unexercised, feedback (timeline)=unrewarding, purgatory / delivery perturbation=unrewarding, recovery_window instrumentation=broken, crash_recovery / post_fault_ops=healthy, timer_effects=healthy, rng_stream_isolation=scaffolding

Policy suggestions: Add a hard gate: no hypothesis (perf, ablate, or arm) may be implemented against a mechanism whose latest utilization counters are all zero. steer.evaluations=0, aos.tape_wins=0, dedup.checks=0, curriculum.lowered_runs=0 today; only 'enabling' hypotheses that flip the mechanism on in general_vr.json should be admissible for those. This alone deletes 5+ live steer hypotheses and the already-merged steer-path-fixed-overhead-ablation's rationale.; Treat multiplier_authority.flipped_configured=0 across a 1x->1000x sweep on 2.1M contested decisions as a defect ticket, not a research hypothesis: the multiplier is not wired to the sampler. Fix or delete it before any further authority/steering work; every 'authority' metric currently emitted is meaningless.; Raise the merge bar to match the measurement floor: require >=3 chunks (>=900 s exposure) for any candidate, require the candidate and baseline to have equal exposure (301 vs 1203 s is not a comparison), and refuse to merge unless |delta| on depth>=6/s exceeds 2x the bootstrap SE (~2-5% at current exposure). Reject any candidate whose depth>=5, >=6 and >=7 deltas disagree in sign.; Replace P(better)>=0.9 with a multiplicity-aware bar (>=0.97, or a mandatory confirmatory second session on a different seed/host slice) and start logging cumulative ladder drift attributable to merges: 29 merges have produced <=0.72% movement on every headline metric, which is consistent with merging noise.; Fix the objective's zero-signal problem: violations=0 over 260,700 runs means the stated primary safety metric has no variance and no hypothesis can be scored on it. Promote violation-signal-density (gain 8, parked) and add a seeded-bug positive control that establishes the depth>=6/s -> detection-rate correlation; without it the entire ladder is an unvalidated proxy.; Neutralize the truncation loophole: either normalize the objective per run (grader-per-run-depth-normalisation, parked at cost 0.7) or exclude grid-short from the union ladder and report it separately. grid-short's 1995 runs/s buys 3.4x the objective from shortened runs alone and currently sets the campaign's headline.; Retire or re-specify h4Rate (0.915 current vs 0.000 reference) and re-derive the reference distribution: P(depth>=4) has drifted 13.7x from reference, so reference-relative claims are no longer meaningful.; Reclaim the aos arm's 20% wall share (~240 s per evaluation) — with tape_wins=0 and config_wins=0 it measures nothing about AOS. Replace it with purgatory-delay-probability-refine-020-025 (cost 0.5) or act-conditioned targeting, both aimed at the demonstrably live purgatory/timer levers where acted_fraction is 0.114 vs 0.396.; Charge propose a real cost and expire the backlog: propose takes 0.26 ms/iter and the pool has grown to 175 with 68 parked and 8 blocked. Auto-close parked hypotheses untouched for N iterations, and reconcile the two disagreeing counters (68/7 vs 66/11) before trusting any pool statistic.; Instrument and cut evaluate overhead: 1815 s/iter of evaluate for 301-1203 s of exposure is 40-72% non-exploring wall. Also investigate the 3-of-15 iterations (5310, 5313, 5314; ~1068 s) that ended in implement with no evaluation at all — a 20% abort rate is a bigger throughput lever than any merged perf hypothesis to date.; Open a defect ticket for recovery_window: 301 of 1408 windows (21.4%) never close and max width (292) is 18x p90 (16); any diagnostic built on window width — including diagnose-top-rung-prefix-events — is currently biased.

## 2026-08-29T14:24:30.246Z

**eval-noise-floor-calibration** (needs_human): A/A probe (spurFiles empty, superFiles = observation notes only) but the harness ran the standard 2-seed protocol, not the requested 6 seeds — so we got one A/A sample per metric, not a per-seed sigma. Even so the falsifier did NOT fire: the no-op spread is 1-2 orders above the deltas recently being credited. Seeds 1000/1001 at ~2.6e5 runs/300s each: d>=4 0.15719 vs 0.16099 (+2.4% rel), d>=5 0.03622 vs 0.03799 (+4.9%), d>=6 0.007741 vs 0.008042 (+3.9%), d>=7 0.0012111 vs 0.0013135 (+8.5%), d>=8 0 vs 3 counts (undefined). h1 .4945/.4851, h2 .4178/.4067, h2b .4299/.4238, h3 .3749/.3581 (abs spread 0.006-0.017), meanPrefixDepth 3.2627/3.2766, runsPerSec 853.6/868.4 (1.7%). The reported objectiveDeltas for this no-op change were depth>=5 -0.0059, depth>=6 -0.0055, depth>=7 -0.0165, depth>=8 -0.0145: that IS the A/A band, ~0.5-1.7% relative, on a diff that touched zero scheduler/spur code. Poisson floor per seed = 1/sqrt(count): 0.5% (d>=4, 41k), 1.0% (d>=5, 9.6k), 2.2% (d>=6, 2.0k), 5.7% (d>=7, ~330), ~58% (d>=8, 0-3). Observed pair gaps vs expected sqrt(2)*sigma: d>=4 2.4% vs 0.7% (3.4x overdispersed), d>=5 4.9% vs 1.4% (3.5x), d>=6 3.9% vs 3.2% (1.2x), d>=7 8.5% vs 8.0% (1.06x). So shallow depths are dominated by a non-Poisson term and deep depths by pure counting. The overdispersion source is visible in the campaign block: round_robin allocates equal WALL time, not equal runs, so arm mix drifts between seeds — aos got 31260 runs/rewardRate 101.0 at seed 1000 vs 34560/167.7 at seed 1001 (+66% reward rate, +10.6% run share), and aos is a deep-tail-rich arm (d>=7 30 vs 50). Practical MDE at the current 2-seed x ~54k(-260k) protocol: ~2% rel at d>=4, ~3% at d>=5, ~7% at d>=6, ~16% at d>=7; d>=8 cannot support any decision at all and should be dropped from the objective until run counts rise 100x. Brute force is dead: detecting a 1% effect at d>=6 needs ~27 seeds (~2.3h each), so the ladder needs variance reduction (paired seeds, fixed-run arm allocation, per-arm stratification), not more seeds. Retro-grade: every auto_merge decided on |delta| < ~2e-2 relative on d>=6/7, or < ~5e-3 on d>=4/5, is noise-graded and should be treated as unverified.

## 2026-08-29T14:51:05.263Z

**stall-abort-quantile-off-identity-path** (closed): Both diagnosed faults from the parent were fixed (progress signal taken from the existing per-delivery 'acted' bit, no State/coverage/curriculum touch; p90-over-sliding-window threshold instead of max) and the gate did fire — yet the result is a clean, large reject at 503,640 runs: depth>=4 ratio 0.889, d>=5 0.899, d>=6 0.911 (primary), d>=7 0.803, d>=8 0.784, i.e. a monotone-in-depth loss, exactly the signature of truncating the tail of long-running productive runs. Critically the compensating throughput never appeared: runs/s ratio 0.948 — aborting runs made the session *slower*, so the per-step quiet-stretch bookkeeping in scheduler/explorer/path cost more than the reclaimed steps returned. The mechanism is therefore falsified on its own terms, not on a confound: the state-identity contamination that killed the parent was absent here and the outcome was still negative. Two structural facts explain why and generalize beyond this hypothesis. (1) There is no budget headroom to reclaim: stepsUsedSum 508.2M against stepBudgetSum 2,097.9M = 24.2% utilization, with 379,496/503,640 runs graded iterations_exhausted — runs are not dying at the step cap in any way that a smarter cap can fix, so the whole 'reclaim wasted steps' family has at most ~a quarter of the budget in play and evidently negative marginal value. (2) A quiet stretch is not a stall. deliveryEffects shows acted/deliveries = 87.3M/155.7M = 56% overall but only 14.3% for biased, 14.8% for delayed, 14.5% for sender-restarted and 2.4% for receiver-restarted deliveries — precisely the fault-injected deliveries that create deep prefixes are the ones that mostly do NOT change state. So the 'acted' bit is anti-correlated with the exploration that produces depth: long no-act stretches are the normal texture of a run that is walking through delayed/restarted traffic toward a deep interleaving, and a p90 taken from *completed* runs (which finish fast, ~1,100 steps) sets a threshold calibrated on the wrong population. That is why d>=7/d>=8 lose ~20% while d>=4 loses 11%. Conclusion: retire the stall-abort family. Any future early-termination work must first demonstrate, offline and read-only, that some observable separates 'this run will never gain depth' from 'this run is quietly working', because the two candidate signals tried so far (node_state_token identity, acted-bit quiescence) are both confounded with depth production itself. Config-param count went 19->20; regression suite failed as expected for a rejected run; lint clean.

## 2026-08-29T15:00:49.463Z

**steer-audit-cost-attribution-config-only** (closed): The gate-toggle attribution ran cleanly (config-only, no rebuild) and produced the table in research/observations/STEER_AUDIT_COST_ATTRIBUTION.md, but it moved the graded objective by -0.3% (need >=5%), so it closed as a measurement, not a win. Key takeaway: turning feedback.steer_audit and stats off did not recover the ~27-point residual between arm-000 (~389 runs/s) and the no-steer-block control (~575), which means the cost is not concentrated in the audit/stats bookkeeping stages the two gates partition; it lives in the always-on steer plumbing (candidate-mask construction / ranking-pass entry, per-selection at 65.5M / 29.3M hits) that no config field currently gates. Also confirms the structural problem flagged at rejudge: with steer.evaluations=0 in the graded config, any steer-path saving is invisible to the objective, so perf work on this path cannot score under the current grader regardless of how much wall-clock it recovers. Neither the observation doc nor general_vr.json changed general config params (19 -> 19), so this was a zero-generality-cost probe. Follow-up work on the steer residual should either add a real gate on the always-on plumbing (compile-time or a single early-out when all weights are zero) or stop being graded as perf until steering is actually exercised in the graded config.

## 2026-08-29T15:13:39.970Z

**steer-zero-weight-early-out** (closed): Null result, cleanly. Hoisted all-weights-zero predicate + branch around candidate-mask construction and the ranking pass: throughput delta -0.0% (0/5% bar), dominance=false, lint clean, general-config params unchanged 19->19, rayon 30, seed 1000, general_vr.json 300s single-arm. No steer_empty_slice evaluations were recorded, so the counters neither confirm nor deny that both stages actually reported skipped -- but a genuine skip of 65.5M mask + 29.3M ranking per-selection hits could not have cost 0.0%; the far more likely reading is that this work was never the residual. Combined with the parent (audit and stats gates priced at ~0), the entire steer block is now excluded as the home of the ~186 runs/s arm-000 (~389) vs no-steer-block-control (~575) gap. Two live explanations remain, and they are testable independently: (a) the gap is not per-selection overhead at all but a workload difference -- removing the steer block changes what gets scheduled/how long each run takes, so runs/s moves for reasons unrelated to CPU spent inside steer; (b) the cost is structural rather than instruction-level -- steer state inflates hot scheduler structs / adds per-scheduler allocations, so the penalty is cache and allocator pressure that a branch cannot skip. Methodological note for the ladder: the control was never validated as apples-to-apples, and three successive attribution hypotheses have now been priced against it and returned ~0. Stop drilling into the steer block until the control itself is characterized; further sub-component early-outs are expected to return 0 as well.

## 2026-08-29T15:38:20.252Z

**timer-effect-steer** (closed): One sequential chunk (255540 runs, 300s, VR/general_vr, seed 1000, 30 rayon threads) rejected on the h2 gate: h2 ratio 0.98 (-0.0099), and h1 -0.0114 / h3 -0.0121 / throughput -0.0130 all move together at ~ -1%, i.e. the coverage loss is essentially the run-count loss, not a change in per-run history quality. Depth moved the other way and monotonically with depth: d>=4 -0.6%, d>=5 +0.4%, d>=6 +5.2%, d>=7 +8.1%, d>=8 +28.4%. Violations 0 on both sides, so nothing was learned about the 76%-at-full-depth regime. Read: lowering admission of inert idle timer firings does trade breadth for depth as designed — the tail deepens sharply and cheaply (1 extra config param, lint clean) — but it costs ~1.3% wall-clock throughput, and the current sequential rule prices any h2 regression above a large primary gain. Termination profile is unchanged in character: 187033/255540 (73.2%) iterations-exhausted, 68394 plan-complete and *all* of them with pending work, 113 deadlocks; steps used 770.8M of 1002.4M budget (76.9%), so budget exhaustion is still the dominant sink and depth gain did not convert into completions. Critical instrumentation gap: the evaluation record carries deliveryEffects and steerAuthority (which is all-zero: steps 0, preferenceExpressed 0, blockedByTimerGate 0) but carries no timer_effects and no timer_steer.{evaluated,lowered,raised} block at all — the stated falsifier ('timer_effects.idle acted_fraction fails to rise') was unobservable in this harness, so we cannot separate 'the multiplier fired often and helped depth' from 'the multiplier fired rarely and the depth shift is scheduler perturbation'. The -1.3% throughput with a per-candidate Beta lookup in score_runnable is the only direct evidence the mechanism is hot. Conclusion: mechanism direction is confirmed on depth, verdict is a gate artifact plus missing telemetry, and the next move is observability before any re-run of the same knob.

## 2026-08-29T16:33:01.899Z

**timer-steer-telemetry-export** (auto_merge): Telemetry plumbing landed (scheduler.rs + util_stats.rs touched, regression/lint green, auto_merge on a noise-level violations delta of +1.9e-6; one violation in seed 1001 arm grid-post-fault-2 at 169.8s), but the export is still incomplete on its own terms: the two serialized evaluation records carry only termination, deliveryEffects and steerAuthority — no timer_effects block and no timer_steer.{evaluated,lowered,raised} keys appear anywhere in utilStats. So the stated deliverable (make timer-admission hypotheses falsifiable) is NOT yet met, and the parent timer-effect-steer remains unevaluable. The secondary question was answered decisively: steerAuthority is all-zero in both seeds (steps=0, preferenceExpressed=0, blockedByTimerGate=0) across 516k runs — that block is unwired, not merely inactive, since steps=0 means the counter is never incremented even once per scheduling step. That is a live falsification of the assumption that existing steer telemetry works; any prior hypothesis judged on steerAuthority evidence was judged on a dead counter. Substantively the run is a null: no behavior change intended, and the metric deltas are consistent with seed noise (h2 -0.002, throughput -0.0017, depth>=6 +0.034 against depth>=8 -0.028 — opposite-sign tail moves, i.e. noise). Also of note: throughput held ~852-866 runs/s and deadlock counts stayed at 132/97, so the added counters cost nothing measurable, confirming the zero-cost claim.

## 2026-08-29T17:49:41.457Z

**timer-effect-steer-late-budget** (closed): Budget-gated damping (late_fraction) inverted the parent's depth profile instead of preserving it: the parent's mid-depth win vanished (d>=6 +5.2% -> -0.15%, d>=5 -0.41%, d>=4 -1.32%) while only the far tail stayed positive (d>=7 +2.9%, d>=8 +9.7%). The stated falsifier fired on the d>=6 clause (gain < half of parent's) but NOT on the h2 clause: h2 came back slightly positive (+0.09%), plus h1 +0.15% / h3 +0.11%, confirming the parent's h2 rejection was throughput-driven run-count loss, not a coverage regression — gating recovered the corpora at the price of the mid-depth gain. Throughput still -0.86% despite skipping the multiplier for the early ~27% of steps, so the scoring-cost story ('cheaper because gated') is wrong; the cost is not in the early phase. Sequential ran to the extended cap: 8 chunks / 2,058,060 runs, d>=7 pGreater 0.977 unresolved. Methodological result worth more than the hypothesis: d>=7 occurs in ~0.8% of runs and d>=8 in ~1e-5, so a tail-only effect of this size is not decidable by this harness at 300s x 8 chunks — any future hypothesis whose sole positive is d>=7/8 will burn a full sequential budget and close inconclusive. Also: utilStats.steerAuthority was all zeros and no timer_steer.{evaluated,lowered,raised,gated_early} block appeared in the exported stats, so the mechanism ran unobserved — we cannot distinguish 'damping rarely fired late' from 'damping fired but is neutral'. The prerequisite in the notes (timer-steer-telemetry-export) evidently had not landed; running this before it wasted the evaluation. Ancillary: violations 0/2.06M across both arms, params +1, lint clean, regression suite not passing.

## 2026-08-29T19:06:47.717Z

**post-boxing-allocation-resweep** (auto_merge): The post-shrink allocation census (research/observations/ALLOC_CENSUS.md) pointed not at a single large-element site but at the simulator core hot loop: edits landed in spur-core/src/simulator/core/{exec,scheduler,state}.rs and produced a 2.5x throughput jump (baseline 614.6 rps -> candidate 1541.2 rps, +150.8%, strict dominance), far above the +5% expected gain. Screen (5400 runs, seed 11) and both promote runs (13500 runs, seeds 11/23) agree at ~1359-1363 rps with 0 violations, 0 unknown, and exploration quality preserved: meanPrefixDepth ~3.28-3.31, maxPrefixDepth 8, h1 ~0.49, h2 ~0.42, h3 ~0.35, h4 ~0.93, depth histogram shape unchanged, general config params 19 before and after. Termination mix (planComplete ~28%, iterationsExhausted ~72%, deadlock <0.1%) and delivery-effect ratios match baseline, so the win is pure throughput per unit work, not a change in what gets explored. Three of three merged perf hypotheses have now come from the >=1032-byte allocation census, confirming it as a reliable, cheap (~4 s) hotspot oracle; the corollary is that after a 2.5x speedup the profile has again shifted and the next census will look at a substantially different distribution, likely with allocation no longer dominant.

## 2026-08-29T20:48:30.100Z

**timer-effect-steer-late-budget** (auto_merge): Re-implemented against current HEAD (Arc<(Record,Lhs)> WaitingReader) and it builds/lints/regresses clean; merged on 2 sequential seeds (281.5k runs each, ~937 runs/s, budget-exhaust rate 73.0% confirming the premise's population estimate). But the effect profile INVERTED relative to the parent: shallow depth up (d>=4 +2.9%, d>=5 +0.09%) while the whole tail the hypothesis was built to preserve went down (d>=6 -2.5%, d>=7 -2.6%, d>=8 -0.4%), with throughput +2.2% and h2 +0.17%. Auto-merge fired on the 'improved: depth>=4' rule even though the primary objective (d>=6) regressed -2.5% — the verdict is a rule artifact, not a win. Interpretation: the parent's depth-tail gain most likely came from the EARLY admission-order perturbation that this variant deliberately removes, not from late-tail inert-timer damping; damping late instead trades deep prefixes for cheaper scoring and more (shallower) runs. Attribution is unverifiable from this evidence: utilStats carries no timer_steer block (fired/damped/gated_early never surfaced) and steerAuthority is all-zero, so we cannot confirm the multiplier engaged at all, nor at what late_fraction — general_vr.json grew 19->20 params but the counters that would prove firing are absent. d>=8 counts are ~330-380 of 281k, so tail deltas are near noise at 2 seeds; no parallel-fidelity confirmation was run.

## 2026-08-29T21:12:37.993Z

**timer-effect-steer-early-only** (closed): Falsified: inverting the budget gate did not recover the parent's tail. 1 chunk / 272,352 runs / seed 1000 rejected on d>=4 ratio 0.99 (-0.022), with d>=5 -0.015 and d>=6 -0.004 (primary); the only positives were d>=7 +0.011 (n=403) and d>=8 +0.143 (n=4 absolute — pure noise, exactly the tail-noise risk flagged in the notes). Throughput -1.1%. So neither the late-only nor the early-only complement reproduces the un-gated parent's lift: both phase gates cost mid-ladder depth, which argues the parent's d>=7/8 'gain' was itself tail noise rather than early admission-order perturbation, and that phase-of-budget is the wrong conditioning variable. Critically, utilStats.steerAuthority came back all zeros (steps 0, preferenceExpressed 0, honored 0, blockedByTimerGate 0) despite timer-steer-counter-emission being a stated dependency — we have no positive evidence the multiplier was ever active, so the measured deltas may be an inert-change seed draw. Config params 19->20 confirms only that the field was added, not that it did anything. Methodological cost: one seed, one chunk, deep-tail bins with n<500 admitted into the decision surface at all.

## 2026-08-29T22:08:59.134Z

**post-2p5x-profile-resweep-cpu** (auto_merge): Merged, +9.4% throughput (1013.4 -> 1108.7 rps, strict dominance; screen 921 rps, promote 924.8/927.8 over seeds 11/23, 0 violations, 19 config params unchanged). The stated falsifier nearly fired and was routed around: no simulator symbol carries >=5% self time except eval::eval (7.44%, irreducible tree-walk dispatch) and exec::execute_common_label (5.16%). Cost is clustered, not concentrated -- allocator 10.73%, Value churn 13.01% (+3.28% memmove), formatting/serialization 7.05%+2.00%. Cluster-level reading still yielded a single lever: state.channels was an imbl HAMT whose 32-entry nodes hold a ~150-byte element (24B ChannelId + two imbl::Vectors) => multi-KB nodes above glibc's 1032B tcache limit, matching the malloc_consolidate/_int_free_chunk signature; worse, every Send/Recv/timer/remote-delivery did get().cloned() -> mutate -> insert, which kept ChannelState Arc-shared and so permanently disabled pop_waiting_reader's Arc::try_unwrap fast path, deep-copying every woken Record. State is never cloned or snapshotted and channels is iterated only in State::signature under XOR, so persistence bought nothing; swapping to an owned FxHasher HashMap with in-place get_mut removed the HAMT walk, the node allocation, the state clone+drop, and the reader deep-copy at once. Method note: the census/profile oracle is now host-limited -- perf is not on the implementer allowlist (the PROFILE.md snapshot was read, not re-taken) and LD_PRELOAD interposition needs a C compiler that is absent, so the byte-histogram half of the plan was replaced by structural reasoning about which containers exceed 1032B. Transferable rule: a persistent container is a liability twice over -- wide nodes above the tcache limit, and forced sharing that silently kills refcount-uniqueness fast paths downstream. Pinned by channel_entry_is_too_wide_for_a_trie_node in core/state.rs.

## 2026-08-29T22:25:33.995Z

**unshare-for-try-unwrap-fast-paths** (closed): Falsifier essentially fired. Static uniqueness audit across simulator/core/{state,exec,scheduler}.rs and values.rs plus the one applied unsharing lever yielded +0.9% throughput (dominance held, params unchanged 19/19, lint clean) against a 5% bar -> closed. The channels win did NOT generalize: that was a single pathological clone-out/insert-back site on a 256-byte Record, not an instance of a repeated pattern. Conclusion for the 13.01% value-churn cluster: it is diffuse, not one blocked fast path. drop_glue::<ValueKind> (3.29) + make_unique (2.77) + Value::new (2.72) + ValueKind::clone (2.49) + EcoVec::drop (1.74) are many small refcount events across many sites, most of which are already reachable at strong_count==1 or are genuinely shared (2+ live holders by construction), so converting individual owners to in-place access moves ~1% at best. Methodological lesson, worth more than the perf result: static reachability reasoning about refcounts predicts *whether* a fast path can fire but says nothing about *how often* it is missed at runtime, and cost is dominated by the latter. Two consecutive edits were justified by the same static argument with opposite outcomes (+9.4% vs +0.9%) -- the argument has no discriminating power. Any further attack on value churn must be gated on measured per-site miss counters, or must change the representation so that a clone is cheap regardless of sharing, rather than trying to make clones not happen.

## 2026-08-29T23:15:58.835Z

**steer-authority-counter-rewire** (auto_merge): Falsifier not triggered: steps is now nonzero. Post-fix seeds 1000/1001 report steerAuthority.steps = 911,844,014 / 921,151,080 — byte-identical to termination.stepsUsedSum in both runs, i.e. the aggregate now reaches serialization (the defect was merge/export wiring, not a cfg gate). Cost: throughput -0.65% (still ~1000 runs/s, 300k runs per seed per 300s), objective deltas within noise, primary +0.0025, h2 +0.0032, violations unchanged at 0, general config params 19->19 (no policy change, as claimed). Regression test spur-core/tests/steer_authority_wiring.rs locks the steps>0 | stepsUsedSum>0 invariant.

Caveat that limits the payoff: steps == stepsUsedSum exactly means the counter carries zero information beyond the existing termination stat — it proves the export path is live but says nothing about the decision site. The four counters that actually matter (preferenceExpressed, preferenceHonored, honored, blockedByTimerGate) are STILL 0 across 601k runs, and the new invariant test does not cover them. So the original blind spot is only half-closed: for every steer/timer hypothesis we still cannot distinguish 'preference source never consulted / dead increment' from 'consulted, never expressed a preference under general_vr.json'. The missing piece is an unconditional denominator at the consultation site. Until that exists, falsifiers of the form 'preferenceHonored rises' remain vacuous, exactly as before.

Secondary observation: this class of bug (all-zero substruct serialized for hundreds of k runs without anyone noticing) is a sweep-able property, not a one-off — the fix pattern generalizes to every UtilStats substruct.

## 2026-08-30T00:01:03.482Z

**steer-preference-denominator-counter** (auto_merge): Falsifier came back UNREADABLE, not negative. The exported utilStats.steerAuthority block after merge is still exactly {steps, preferenceExpressed, preferenceHonored, honored, blockedByTimerGate} across both seeds (595k runs) — preferenceConsulted and preferenceSourceAbsent do not appear as keys at all. So we cannot say the consultation site is dead; we can only say the new counters never reached the artifact. Note the diff touched scheduler.rs, util_stats.rs and tests/steer_authority_wiring.rs but NOT explorer.rs, even though the hypothesis explicitly called for it: the wiring test asserted on in-process stats and passed while the export path silently dropped the fields. This is the third consecutive steer-counter experiment lost to the export layer (parent rewire recovered only `steps`, which still equals termination.stepsUsedSum exactly — 923,697,217 and 918,478,356 — i.e. `steps` is a re-derivation of an existing quantity and carries zero new information about steer). Conclusion: the steerAuthority serializer is a hand-maintained key list, and any hypothesis that adds a field without editing it is structurally vacuous. Objective side: merged auto_merge as non-inferior enabling; primary -0.0151, throughput -0.0097, depth>=7 pooled +0.047 / depth>=8 pooled +0.022, one violation at seed 1001 (grid arm, 217.4s). A pure-counter change cannot cause these, so this run also calibrates the 2-seed noise band on primary/throughput at ~1-1.5% — smaller claimed effects in this ladder are not resolvable at this fidelity.

## 2026-08-30T00:21:51.982Z

**throughput-to-depth-reinvestment** (needs_human): Refuted the premise, with an in-run natural experiment. The arm was implemented by ADDING a sixth campaign arm (grid-deep, overlay max_iterations=24000) rather than re-tuning existing knobs, which (a) made the rate stratum incomparable (candidate pools 5 grid arms vs baseline 4) -> needs_human, and (b) diluted fixed wall-clock exposure: 919 runs/s vs baseline ~1360, and grid-deep itself only 7072 runs at 49.4 runs/s vs grid-short's 133120 at 553 runs/s. All pooled depth deltas went negative (depth>=4 -0.067 ... depth>=8 -0.147) and h2 -0.0018 - but that is an arm-mix artifact of the pooled fraction metric, not a depth effect. The real signal is the per-arm tail across a 16x iteration-budget range at identical scheduler code: depthAtLeast[7]/runs = grid-short(1500 iters) 0.00126, grid(6000) 0.00153, grid-deep(24000) 0.00141, aos 0.00162. Flat. 4x more iterations bought zero additional depth tail per run. Utilization also did not move: stepsUsedSum/stepBudgetSum 906M/1185M = 76.4% (baseline ~76%), iterationsExhausted 202230/276192 = 73.2% (baseline 72%), meanPrefixDepth 3.278 (baseline 3.30). maxPrefixDepth 9 vs 8 is a 3-run tail event at 276k runs, not a regime change. Conclusion: 'iterationsExhausted at 72%' is NOT evidence that budget is binding - runs exhaust iterations at a saturated ~76% step utilization regardless of how large the budget is, because prefix depth is limited by whatever policy proposes/extends prefixes, not by the step or iteration ceiling. Also a protocol lesson for the loop: arm-set changes and overlay changes must never be bundled, and pooled depth fractions are not a valid cross-config statistic when the arm mix differs; compare per-arm at equal per-arm exposure. Zero violations in 276k runs, regression+lint clean, so the config is safe - it just does nothing useful.

## 2026-08-30T00:27:47.964Z

### Audit @5340
Regression (579.4) + rejudge (196.7) = 776.1 of 1220.0 accounted ledger units = 63.6% of tracked time, versus evaluate at 386.6 = 31.7%. Nearly two-thirds of tracked wall time is re-verification of things already decided, not acquisition of new evidence. Separately, 'implement' does not appear in the ledger at all despite being the single largest per-iteration phase in the last-15 table (mean 675 s/iteration: 512 s model/think, 43 s smoke, 40 s test, 29 s build, 3.6 cargo builds and 17.7 edits per iteration) — i.e. ~10.1 h of implement time across the last 15 iterations is invisible to the budget accounting. Hypothesis yield is low: 205 total, 35 merged (17.1%), 74 closed + 80 parked = 154 (75.1%) producing nothing. Within the visible 40-hypothesis window, ~12 (30%) target the steer/timer-steer subsystem, which utilization shows is 100% inert (see findings); three of those (steer-preference-denominator-counter, steer-authority-counter-rewire, timer-steer-telemetry-export) were actually merged, so ~9% of all merges landed instrumentation on a code path that never executes. Campaign wall budget is also concentrated wastefully: round_robin gives the aos arm 20% (60 s of every 300 s chunk) despite aos.tape_wins = 0 and aos.config_wins = 0, making it a duplicate of the grid arm at full price.

Goodhart: Primary objective is 'rung events per explore-second', which is directly maximized by shortening runs. The grid-short arm (max_iterations 1500 vs 6000) posts 2203 runs/s and 86.90 d>=6/s versus plain grid at 663-694 runs/s and 29.49 d>=6/s — a 2.95x objective advantage bought purely by truncating runs, not by exploring deeper. Any future arm that cuts max_iterations further will 'win' the objective while reducing per-run search depth.; runsPerSec has grown 6.97x over reference (142.4 -> 992.0) and P(depth>=4) 13.7x (0.034 -> 0.466), yet the one experiment that tested whether throughput converts to depth (throughput-to-depth-reinvestment, 259,552 runs) returned P(depth up) = 0.000 at every rung. The loop has been optimizing the denominator of its own metric.; violations — the only rung that corresponds to actually finding a bug — moved 0 (reference) -> 2 (baseline) -> 1 (latest), while every proxy rung exploded. The proxy and the goal have visibly decoupled, and the highest-gain hypothesis in the pool addressing this (violation-signal-density, gain 8/cost 4) is parked.; h4Rate is saturated at 0.916 (reference 0). A rung hit in 92% of runs carries ~0.29 bits and cannot discriminate between candidates, but it still occupies a ladder row and can be cited as 'improved'. h1/h2/h2b/h3 are all pinned within 0.02-0.03 of reference (0.489/0.491, 0.388/0.412, 0.417/0.424, 0.342/0.361) — five of nine ladder rows are effectively constant and contribute no decision signal.; Three merged hypotheses (steer-preference-denominator-counter, steer-authority-counter-rewire, timer-steer-telemetry-export) add counters to a subsystem with steer.evaluations = 0. The counters now report large non-zero numbers (steer_authority.steps = 4,781,571; preference_consulted = 14,344,593) that look like activity but are 100% no-op steps: preference_source_absent = 14,344,593 = preference_consulted exactly, and preference_expressed = honored = audited = 0. This is instrumentation that manufactures the appearance of a live mechanism.; The latest merge shows the primary objective (d>=6/s) DOWN 6.3% and d>=5/s down 3.0%, but the summary row leads with 'violations 2 -> 1' — a 1-event move on an unpowered metric being used to justify a regression on the powered one.

Utilization: steer (feedback.steer / steer_terms)=broken, multiplier_authority=unrewarding, dedup=unexercised, aos=unexercised, curriculum=unexercised, feedback (timeline scoring)=unrewarding, purgatory=healthy, delivery_effects: sender_restarted / receiver_restarted=unrewarding, timer_effects (idle timers)=unrewarding, recovery_window=broken, rng_stream_isolation=scaffolding, post_fault_ops=healthy

Policy suggestions: Add a hard pre-implement liveness gate: reject any hypothesis whose named buildsOn mechanism reports zero activity in the latest utilization dump. steer.evaluations = 0 and steer_authority.audited/preference_expressed/honored = 0 should have blocked ~12 visible steer/timer-steer hypotheses and prevented 3 of the 35 merges. This is exactly steer-mechanism-liveness-guard (gain 7/cost 2, parked) and grader-steer-path-unreachable-audit (gain 3/cost 0.5, parked) — promote both now, they are the highest-leverage items in the pool by a wide margin.; Fix or formally disable steer before spending another iteration on it: all four steer_terms weights are 0 in the eval config, which is why 4,781,571 of 4,781,571 steps hit the empty-slice path. Either set non-zero weights and re-baseline, or set feedback.steer = false and delete the steer hypotheses wholesale. Do not merge another steer counter.; Change the primary objective so it cannot be gamed by run truncation. 'Rung events per explore-second' hands grid-short a 2.95x advantage (86.90 vs 29.49 d>=6/s) purely for cutting max_iterations 6000 -> 1500. Normalize per simulated step or per CPU-second, or fix max_iterations across all arms and move run length to an explicit ablation arm.; Stop reporting violations in the ladder as if it were decision-grade. At 2 events per 1201 s the Poisson CI spans ~30x. Either promote violation-signal-density (gain 8/cost 4, parked — the highest-gain hypothesis in the pool) to raise the event rate to at least ~100 per chunk, or demote violations to a monitored-not-optimized row so it stops being used to justify regressions on d>=6/s.; Require merge decisions to compare at equal exposure. The last merge accepted -6.3% d>=6/s and -3.0% d>=5/s measured at 300 s against a 1201 s baseline. Mandate the same chunk count on both sides, or require the auto_merge gate to clear a measured A/A band — promote eval-aa-noise-gate (gain 7/cost 2, parked) and reopen eval-paired-seeds-fixed-run-arms (gain 6, closed) which claimed a 3x variance reduction.; Drop the aos arm from the campaign. tape_wins = 0 and config_wins = 0 means it is measuring nothing aos-specific; it duplicates grid within noise while taking 60 s of every 300 s chunk. Reallocate that 20% to deep-tail-power-arm (gain 4/cost 1.5, parked) to make d>=7 and d>=8 decidable — currently d>=8 has ~300 events per chunk (SE ~5.8%) and is unusable.; Reclassify throughput-to-depth-reinvestment from inconclusive to closed-negative and revoke its 2 remaining resumes. P(up) = 0.000 on d>=4, d>=5 and d>=6 across 259,552 runs is a decisive result; keeping it resumable budgets more compute to re-derive it.; Cap regression + rejudge, which are 63.6% of accounted ledger time (776.1 of 1220.0) versus 31.7% for evaluate. Run full regression on a fixed cadence (e.g. every 5th merge or on ladder-row movement > the A/A band) rather than per merge, and cap rejudge (196.7, 16.1%) at one re-judge per hypothesis.; Add 'implement' to the time ledger. It is the largest per-iteration phase in the last-15 table (~675 s/iteration, of which 512 s is model/think) and is entirely absent from the budget accounting, so no cost/benefit ratio in the hypothesis pool is trustworthy.; Retire or repair multiplier_authority: 4,781,451 decisions, 2,111,158 contested, flipped_configured = 0, and zero flips across a 1.0-to-1000.0 sweep. A knob with no authority at 1000x should be deleted along with emit_multiplier_authority, not instrumented further.; Prune saturated and pinned ladder rows from the decision set. h4Rate = 0.916 is saturated; h1/h2/h2b/h3 sit within 0.03 of reference and have not moved in the visible history. Five of nine rows contribute no discrimination but do contribute multiple-comparison surface for cherry-picking.; Open a defect on recovery_window: 301/1408 (21.4%) windows never close, and max = 292 is well under width_cap = 1024, so this is not a cap artifact. Any statistic conditioned on window closure is silently biased.; Raise the utilization-dump sample size or stop drawing mechanism conclusions from it. It is 1080 runs against a 297,728-run ladder; effects like the 50-event receiver_restarted acted count are being read as mechanism evidence at ~275x less data than the metrics they purport to explain.; Set an explicit pool-hygiene policy: 80 parked and 74 closed against 35 merged means 75% of hypothesis-generation effort is discarded. Cap the parked queue (e.g. 20) and force parked items to either be promoted or deleted, so propose does not keep regenerating variants of dead-mechanism ideas.

## 2026-08-30T01:10:17.156Z

**steer-stats-export-parity-test** (auto_merge): Falsified as stated, but it located the real drop site. The Rust side was already fine: SteerAuthorityStats (spur/spur-core/src/simulator/util_stats.rs:1411) is derive-Serialize with preference_consulted/preference_source_absent present since the parent, and the new spur-core/tests/stats_export_parity.rs passes on today's tree for BOTH export paths (CLI utilization dump and the per-arm campaign `counters` accumulator) — no hand-maintained key list, nothing to remove in explorer.rs. The only diff that landed was the test plus research/observations/STEER_AUTHORITY_COUNTERS.md. The field-drop is downstream, in harness TypeScript: research/orchestrator/src/evaluate.ts:161 `utilSubset` copies exactly five of the twelve steerAuthority names by hand, and research/orchestrator/src/schemas.ts:201 validates against a closed z.object of the same five — which is why every evaluation record in this thread, including this one, shows steerAuthority as {steps, preferenceExpressed, preferenceHonored, honored, blockedByTimerGate} and nothing else. Read straight from utilization.json instead, the block is not dead: steps 79.4M, preference_consulted 238.2M, preference_source_absent 238.2M (exactly three consultations per scheduling step, every one finding no source), everything else 0. So the all-zero authority block under scheduler_configs/loop/general_vr.json is a policy fact — steer_terms weights are all zero so the queue audit never runs and `audited` is legitimately 0 — not broken wiring. Three prior iterations (5334/5338/5339) read that backwards. Objective deltas were noise (primary depth>=6 -0.0087, h2 -0.011, throughput -0.36%, violations 0 across 594k runs, 19->19 config params); auto-merged as enabling/non-inferior on the test artifact alone.
...
## 2026-08-30T02:56:38.092Z

**coverage-guided-fault-placement** (needs_human): Null at screen stage, and formally uninterpretable under its own pre-registration. Two sequential seeds (1000/1001, 300s, 302,912 + 297,056 runs, 14 rayon threads): every depth rung moved inside seed-to-seed noise and with no consistent sign — depth>=4 -2.4%, >=5 -3.9%, >=6 -3.1%, but >=7 +1.7% and >=8 +5.2% (that top rung is 407 and 399 runs, i.e. counting noise). Throughput -0.75%, h2 +0.5%, primary +2.5e-6 driven entirely by a single violation that landed in grid-short in both seeds (first hit at 22.6s in one seed, 239.6s in the other — timing spread that wide is a lottery, not a mechanism), which is why the verdict is needs_human rather than a clean negative. Depth is once again budget- and signal-invariant: the ladder did not care which node we chose to crash, at least not with this tag. Two things block a real verdict. (1) The pre-registered fire criterion (candidates_ge2 >= 50% of decisions, diverged_from_uniform >= 10%, distinct_tags >= 20) cannot be checked: the recorded utilStats blocks contain only termination / deliveryEffects / steerAuthority, and none of the fault_coverage.* counters appear in either evaluation record. So we do not know whether the argmin ever diverged from the uniform pick — the arm may have been a no-op the whole session. (2) The touched spur files are coverage.rs, curriculum.rs, explorer.rs, path.rs, util_stats.rs — no fault-injection module in the diff — which is weak evidence the change landed on the queue/coverage side rather than at the crash-injection site the hypothesis specified, i.e. possibly the same inert channel as every prior coverage attempt (preferenceExpressed still 0 over 907-952M steps, as expected with novelty off). Conclusion: do not retry a placement-coverage variant until the counters are visible in the eval record and the candidate-set size at crash decisions is known. If candidates_ge2 lands under 50%, the crash subject is structurally near-determined and the whole placement family closes. Standing lesson reinforced: any gated feedback hypothesis whose fire criterion depends on counters that the harness does not serialize into the evaluation JSON burns a full 10-minute screen and returns nothing decidable — instrument the readout before spending the session.

## 2026-08-30T03:47:47.757Z

**purgatory-skip-holds-into-down-receivers** (auto_merge): Ablation landed and was neutral, not load-bearing: with hold_down_receivers=false wired into general_vr.json, two 300s seeds (1000/1001, 300480 + 299360 runs) gave primary depth>=6 -0.0109 (pooled -0.0262), depth>=4..>=8 all within the null band, throughput -0.008, violations ~0 — auto-merged as non-inferior. The red team's inversion worry is refuted at this dose: holds into already-down receivers are NOT the mechanism that manufactures receiver-restarted-in-the-meantime, since removing them cost no depth. But the acted-fraction pricing story is NOT vindicated either, because the suppressed population is tiny: receiverRestarted deliveries are 571182/194.9M = 0.29% of deliveries (seed 1000; 555859/195.1M seed 1001) at 2.25%/2.16% acted vs senderRestarted 14.4% and all-deliveries 36.6%. A ~0.3% subpopulation cannot move the ladder in either direction, so ~zero delta was near-guaranteed by volume and the test had almost no power to discriminate 'absorbed surface' from 'load-bearing'. Also: the promised split counters (holds_down_receiver / holds_up_receiver / passthrough_down_receiver) do not appear in the reported utilStats payload (only termination / deliveryEffects / steerAuthority are surfaced), so the headline promise — first measurement of how purgatory's holds divide — was not actually harvested. Operational lesson for the family: size the target population from deliveryEffects BEFORE spending 2x300s; the material purgatory surface is `delayed` (22.7M deliveries, 11.6% of all, 11.5% acted), three orders of magnitude bigger than what this ablation touched. Purgatory's 11-point d>=6/s gap still lives entirely in the up-receiver half and remains undecomposed.

## 2026-08-30T04:37:26.384Z

**stale-delivery-acceptance-distance-census** (auto_merge): Merged (auto_merge, enabling/non-inferior): the per-node event counter + restart-boundary distance histogram now rides inside deliveryEffects, confirming again that piggybacking on an already-serialized block is the reliable way to get new instrumentation to the eval record. Two 300s sequential seeds (296,640 and 298,400 runs, ~988-994 runs/s, 0 violations, 14 rayon threads) show the added per-node u32 increment in the event-execution path cost throughput -0.8%, i.e. the 'measure-only' patch is not free but is within noise of the arm mix; depth>=6 primary +0.0064 and depth>=5/6/7/8 pooled all positive, so no ladder harm. The census inputs are stable across seeds: senderRestarted 288,925/291,958 deliveries at 14.6-14.9% acted vs receiverRestarted 548,370/549,021 at 2.16-2.20% acted — the 7x sender-vs-receiver asymmetry replicates tightly, and receiver-restarted deliveries are ~1.9x more numerous than sender-restarted while being ~7x less likely to act. Baseline for comparison: all deliveries 39.0% acted, biased 11.9-12.1%, delayed 12.1-12.2%. Note the eval record here carries only the aggregate deliveryEffects rollup; the per-bucket distance histogram lands in research/observations/ACCEPTANCE_DISTANCE.md via acceptance_distance.py, so the shape of the curve (steep vs flat) must be read from that report, not from these metrics. The h2 rate itself moved -0.0065, consistent with the counter touching the hot path rather than with any scheduling change. Config surface grew by one param (20->21).

## 2026-08-30T05:26:47.123Z

**counters-piggyback-serialized-block** (auto_merge): Refuted as specified, but with a sharper positive: the eval record's utilStats.deliveryEffects DID arrive and now carries senderRestarted/receiverRestarted sub-blocks with nonzero deliveries+acted on both seeds (1000: 297389/43246 sender, 551728/12493 receiver; 1001: 288702/42581, 548281/12040) — so the projection is NOT a field-level allow-list, new fields inside an already-serialized block do reach the record. What did NOT appear are the five flat scalar names the hypothesis named (crashes_total, recoveries_total, stale_sender_deliveries_after_recovery, etc.): the shipped diff instead expressed fault readout as delivery-keyed sub-blocks under deliveryEffects. So the family fault-counter-readout-plumbing stays OPEN and the negative that would have closed it did not materialize; the correct read is that the emit path works when a counter is attached to a delivery event, and only counters attached to non-delivery events (crash/recover ticks, prefixExtension, fault_coverage.*) are still unrepresented — those blocks remain absent, consistent with them being separate top-level keys that the projection drops at BLOCK granularity. Rule-8 base rates now exist for free: restart-adjacent deliveries are ~0.44% of all deliveries (297k+552k of 194.7M) and their act rates diverge sharply from baseline — senderRestarted acts at 14.5% vs receiverRestarted at 2.3% vs all-deliveries 38.3% — i.e. delivering to a just-restarted receiver almost never changes state (it is nearly pure waste), while delivering from a restarted sender is 6x more productive than that but still 2.6x below baseline. That asymmetry is a directly actionable steering signal and was invisible before this patch. Cost side: near-flat, primary +0.0017, throughput -0.2%, stratified +1, violations 0, pooled depth tails down 2-6% (deepest bins noisy at n=5 runs at depth 9) — consistent with pure instrumentation overhead plus seed noise, auto-merged as non-inferior enabling. steerAuthority remains all-zero (preferenceExpressed=0 across 922M steps), which is a separate dead-readout that this patch did not touch and which now looks like the next real plumbing gap.

## 2026-08-30T05:50:09.389Z

**steer-term-crash-after-timer-sends-single-arm** (needs_human): Plumbing is alive, effect is nil. Setting steer_terms.crash_after_timer_sends=1.0 in one arm flipped session steerAuthority.preferenceExpressed from a hard 0 (over 956M steps across all prior epochs) to 24385 over 910.5M steps, with preferenceHonored=4705 (19.3% of expressed) and blockedByTimerGate=0. So the feedback lens is not broken code — it was inert only because every weight was literally zero, and one nonzero weight suffices to make score_runnable express a preference. Falsifier for the plumbing hypothesis therefore fails; the term itself is confirmed reachable. But expression is vanishingly rare: 24385/910.5M = 2.7e-5 per step, and only ~1/5 of expressions are honored, i.e. the predicate almost never discriminates among runnable events, and when it does the scheduler usually cannot act on it. Depth is unmoved: the new arm's depth>=7 rate is 374/40032 = 9.34e-3 vs baseline grid 389/41888 = 9.29e-3 (>=8: 62 vs 53; >=9: 0 vs 2) — indistinguishable at these counts. rewardRate 189.1 vs grid 196.7 (-3.9%), inside the spread of the untouched arms (no-purgatory 193.5). Violations 0, unknown 0, regression+lint clean. Pooled objective deltas are noise-shaped (depth>=7 +0.058 on 368 total runs at that depth; depth>=8 -0.0037; h2 -0.0041). Two methodological facts, both costly: (1) steerAuthority is serialized at SESSION scope only, not per arm, so the pre-registered within-session control (expressed==0 on the four untouched arms) is UNTESTABLE from this record — the 24385 cannot be attributed to grid-steer-timer-crash except by the prior that the other four arms carry all-zero weights; (2) swapping the arm id set (aos -> grid-steer-timer-crash) broke the rate stratum (candidate pools 5, baseline pools 4), forcing needs_human, exactly as the parent's park note warned. Overlay-swap-not-arm-add was honored in count but not in id set; the stratifier keys on ids, not cardinality. Net: the cheapest possible non-vacuous steer test cost a full 300s session and returned an uncomparable objective plus an unattributable counter. Fix the two instrumentation faults before spending more wall clock on steer weights.

## 2026-08-30T06:36:58.722Z

**steer-steps-provenance-ablate** (auto_merge): Inconclusive-by-instrumentation, but strongly suggestive. Both sequential runs (seed 1000: steps=882762276 vs stepsUsedSum=882762276; seed 1001: 903589032 vs 903589032) show steerAuthority.steps STILL equal to termination.stepsUsedSum to the digit after the claimed split — now six consecutive runs of exact equality. More telling: steerAuthority.stepsTotal does not appear in utilStats at all; the exported steerAuthority object is unchanged in shape {steps, preferenceExpressed, preferenceHonored, honored, blockedByTimerGate}. So the ablation could not actually be read out: either (a) the new counter was added in util_stats but the JSON export path is a hand-maintained allowlist that silently drops unknown fields, or (b) the split never reached the exported field. The declared falsifier (steps < stepsTotal strictly) was therefore never evaluated against observable data, and the in-repo test in steer_authority_wiring.rs passing is not evidence — it can assert over in-process structs that never reach the export. Corollary: the presumed dependency 'steer-stats-export-parity-test' did NOT actually land as a general parity guarantee, only for the fields already exported. Independent of the alias question, the whole steerAuthority block remains inert on the hot path: preferenceExpressed=preferenceHonored=honored=blockedByTimerGate=0 across 293760+297472 runs and ~1.79e9 steps. A denominator of 8.8e8 with a numerator of exactly 0 means the steer decision site is either unreachable under general_vr.json or reachable but always short-circuiting before the express point — no further counter-plumbing on top of a zero numerator can discriminate those. Objectives: non-inferior flag-off auto-merge, primary -0.0065, throughput -1.8%, h2 -0.0039, violations 0, depth>=7/8 positive (+0.017/+0.094) but on 383/5 and 390/3 runs at those depths — pure tail noise, not signal. Cost paid: ~10 min sequential exposure for zero decidable information. Methodological lesson for the epoch: any hypothesis whose falsifier is 'compare two counters' must first prove both counters are visible in the exported artifact — assert on the run's own JSON, not on a unit test, before spending a 300s campaign.

## 2026-08-30T06:41:46.503Z

### Audit @5350
Mean iteration ~2783 s of tracked phase time. Evaluate (1562.7 s, 56%) plus regression (530.0 s, 19%) = 75% of every iteration is spent re-measuring, versus 15.8% on implement (440.9 s) and effectively 0% on propose (0.45 ms — proposal is amortized/cached, only iteration 5346 and 5344 paid ~325-360 s). Evaluate is a fixed 300 s campaign x 4 chunks + overhead; regression is a near-constant 530 s tax that has produced 0 violations for the entire visible history. Rejudge at 181.8 s (6.5%) is pure meta-overhead. Within implement, 451 s/iteration is model/think and only ~26 s build + 44 s smoke + 33 s test — i.e. the implement phase is thinking, not building (16 edits, 3.9 cargo builds per iteration). Recent iterations 5337-5349 run 23-50 min each; at ~50 min/iteration the loop buys roughly one hypothesis verdict per hour, three quarters of which is spent confirming a baseline that has not moved.

Goodhart: Primary objective is a rate (events per explore-second), so throughput inflation substitutes for exploration quality: runsPerSec is 1002.8 vs reference 142.4 (7x), while meanPrefixDepth is 3.27 and saturating. The grid-short arm (max_iterations 1500) triples runs/s to 2223 and posts the best d>=6/s (90.06, ~3x the next arm) purely by making runs shorter — the metric rewards truncating runs, not finding deeper prefixes. If arm allocation ever becomes reward-driven, the campaign will collapse onto grid-short and optimize run brevity.; Reward is termination_completed, which pays for runs that finish, again favoring short/cheap runs; grid-short's reward rate 557.9 vs 160-216 for every other arm confirms the reward channel is measuring run length, not bug-finding.; P(depth>=4) is 0.464 vs reference 0.034 (13.6x) and P(depth>=5) 0.159 vs 0.002 (80x), yet violations remain exactly 0 across every arm and every chunk. Depth has been driven two orders of magnitude past the reference without producing a single new violation — the proxy has decoupled from the goal it proxies for.; h4Rate 0.914 against a reference of 0 is a metric that has been maxed out and carries no remaining information; it is still printed in the ladder as if it were a live signal.; A visible cluster of hypotheses optimizes the readout rather than the system: counters-piggyback-serialized-block (merged), steer-stats-export-parity-test (merged), steer-preference-denominator-counter (merged), fault-counter-readout-plumbing, orchestrator-util-subset-passthrough, steer-authority-dead-readout-audit, steer-term-in-existing-arm-id ('carry the steer term inside an existing arm id so the rate...'). Merging serialization/plumbing work counts as progress in the merged tally (40-41) while the metric ladder is flat between baseline and latest merged on every row.; Latest merged is worse than baseline on 8 of 9 rate rows (d>=4/s 465.4->460.8, d>=5/s 159.2->156.4, d>=6/s 37.3->35.7, d>=7/s 7.78->7.47, h1 0.490->0.480, h2 0.413->0.404, h2b 0.425->0.417, h3 0.365->0.355) and yet was merged; the acceptance rule is not enforcing non-regression on the primary objective.; Exposure asymmetry: baseline is scored at 1201 s and the merged candidate at 300 s, so the two columns compared side-by-side in the ladder are not statistically commensurate.

Utilization: aos=unexercised, dedup=unexercised, steer=broken, steer_authority=broken, multiplier_authority=unrewarding, purgatory=healthy, crash_recovery / recovery_window=healthy, curriculum=unexercised, feedback (timeline scoring)=unrewarding, rng_stream_isolation=scaffolding, ordered_h3=healthy, post_fault_ops=healthy, delivery_effects (acted_fraction instrumentation)=healthy

Policy suggestions: Freeze all steer-adjacent hypotheses until the dead path is fixed or the mechanism is deleted. steer.evaluations = 0, steer_authority.preference_source_absent = 100% of 14.4M consultations, steer_terms all 0 in config. At least 8 pool entries (steer-term-in-existing-arm-id, steer-authority-per-arm-serialization, steer-authority-dead-readout-audit, steer-steps-provenance-ablate, steer-terms-nonzero-probe-arm, orchestrator-util-subset-passthrough, timer-steer-counter-emission, timer-steer-counter-attribution, steer-mechanism-liveness-guard) are downstream of a path that never runs. Exactly one of them should proceed: set a steer_term nonzero and confirm evaluations > 0. The rest are unmeasurable by construction and should be blocked, not parked, so the proposer stops re-surfacing them.; Delete multiplier_authority or prove it live. A 1000x multiplier flipping 0 of 2,136,219 contested decisions is a definitive negative; the sweep has already run the ablation. Remove the mechanism and its counters rather than paying for emit_multiplier_authority on every run.; Promote dead-counter-audit-invariant (currently parked, gain/cost 4/2) to blocking and run it before any further enabling hypothesis. A CI check that fails when a UtilStats substruct is all-zero would have caught steer, aos, dedup, curriculum and multiplier_authority automatically, and would have pre-empted several merged serialization hypotheses whose only effect was to serialize zeros more faithfully.; Add a hard non-regression gate on the primary objective before merge. The latest merged change is worse than baseline on 8 of 9 rate rows (d>=6/s 37.32 -> 35.74, -4.2%) and merged anyway. Require the candidate to be non-inferior at the same exposure, and stop displaying a 1201 s baseline next to a 300 s candidate as if they were comparable — score both at 1200 s or neither.; Fix the throughput-inflating objective. Rung events per explore-second lets grid-short win by truncating runs (max_iterations 1500 -> 2223 runs/s, 90.06 d>=6/s, reward rate 557.9 vs 160-216 elsewhere). Either normalize the objective per unit of simulated work rather than per wall-second, or cap the runs/s credit, or drop grid-short. Otherwise any reward-driven allocator will converge on the shortest possible run.; Reclaim the 20% wall share spent on the aos arm. aos.tape_wins = 0 and config_wins = 0, so the arm is grid with extra machinery. Either write an enabling hypothesis that makes tape_wins nonzero first, or replace the arm with deep-tail-power-arm (parked, 4/1.5) to buy statistical power at d>=7 where SE is currently ~2% and per-arm slices are too thin to decide anything.; Attack the 530 s regression phase. It has returned 0 violations across the entire visible history at 19% of every iteration's wall time. Run it every N iterations (matching the audit-every-5 cadence) or on merge only, and reinvest the recovered ~2100 s per 5 iterations into a fifth evaluation chunk, which is worth more than a redundant violation check on the metric that actually gates decisions.; Apply a multiple-comparison correction or a pre-registered shortlist. 229 hypotheses tested at nominal p<0.05 implies ~11 spurious separable calls; 40-41 merges have accumulated against a ladder that is flat. Either tighten the reject threshold, or require any merged effect to replicate in an independent chunk set before it enters the baseline.; Retire h4Rate from the ladder. It sits at 0.914 against a reference of 0 and has not moved; it consumes a row and implies signal where none remains. Same for the violations row unless the regression cadence changes — both are constants being reported as metrics.; Redirect the freed budget to the two mechanisms with demonstrated live effects: purgatory (ablation moves d>=6/s 28.48 -> 15.32, a 46% swing, the largest measured effect available) and receiver_restarted deliveries (acted_fraction 0.0274 vs 0.396 all-deliveries, a 14x suppression on 2006 samples). receiver-restarted-delivery-deprioritize and acceptance-window-targeted-delay-placement are both parked while readout-plumbing hypotheses were merged; that ordering is inverted.; Cut the implement phase's think-to-build ratio. 451 s of model/think against 26 s build, 44 s smoke and 33 s test per iteration, producing 16 edits and 3.9 cargo builds. Cap implement turns well below the current 120 for enabling/plumbing hypotheses, which are mechanical and should not cost 7.5 minutes of reasoning each.

## 2026-08-30T07:00:18.208Z

**recovery-anchored-to-own-inflight-sends** (closed): Rejected at the first sequential chunk (284640 runs, 1 chunk, ratio 0.99 on depth>=4). The mechanism was NOT inert — it is the first member of this family to move anything — but it moved the ladder the wrong way: every depth band regressed, monotonically worse with depth (depth>=4 -0.043, >=6 -0.062, >=8 -0.070 per run; pooled -0.046/-0.077/-0.085), throughput -3.8%. Meanwhile the hazard rates it targets went UP (h2 +0.015, h3 +0.012, h1 +0.008). So the predicate did what it claimed — recovering a node while its own pre-crash sends are undelivered manufactures more h1/h2/h3 events — and the ladder does not care: those extra hazards do not convert into deeper prefixes, they displace the schedule diversity that does. This is the seventh falsification on the sender-side/receiver-side race and the FIRST to attack the recovery event rather than the messages; the 'moves messages, not the event' escape hatch is now closed, and with it the whole 'acted-asymmetry' rationale that generated the family. The asymmetry itself replicated cleanly under the new arm (senderRestarted 46461/296032 = 15.7% acted vs receiverRestarted 10905/526921 = 2.1%, ~7.6x), so the measurement was right and the inference from it was wrong. Two confounds left standing: (a) per-run depth loss cannot be explained by the -3.8% throughput, so the loss is in the schedule itself, but we cannot tell whether it is THIS predicate or ANY reweighting at the recovery-selection site — steerAuthority (173896 expressed / 32740 honored, 18.8%) is a shared counter and not attributable to this term; (b) only one weight value (1.0) was evaluated, so 'wrong sign' and 'right sign, wrong magnitude' are not separated — though the monotone-in-depth damage argues against magnitude. Practical rule for the pool: an add whose only pre-registered fire criterion is a hazard-rate/acted-fraction ratio is under-specified; h-rate gain and primary-objective gain have now diverged in sign on the same run.

## 2026-08-30T07:58:37.158Z

**hazard-rate-vs-primary-decorrelation-audit** (needs_human): The audit never ran. Verdict was needs_human ('meta changes require human review in v1'), and the loop nonetheless spent two full 300s sequential campaigns (seeds 1000/1001, ~591k runs) on an unchanged spur (460184a) — the pre-registered 'zero simulator spend, expectedCost 0.4' was structurally impossible: v1 has no offline/dry execution path, every queued hypothesis gets the standard campaign regardless of kind. So the deliverable (rank correlation + sign-agreement table over stored objectiveDeltas) does not exist, and the queueing rule it was meant to justify is still unwritten.

What we got instead is more useful than expected: a null-diff noise floor. Because the diff was empty, the recorded objectiveDeltas are pure seed/schedule variance under the live grader (ta:08b1b45+porc:ebf06c5, 14 rayon threads, general_vr.json). Measured null band: primary +0.00231, depth>=4 -0.00031, depth>=5 -0.00164, depth>=6 +0.00231, depth>=7 -0.00671, depth>=8 -0.00985, h2 +0.00354, throughput -0.00025, panelZ 0.036, violations 0. Two readings: (a) |primary| <= ~0.003 and |h-rate| <= ~0.004 are indistinguishable from nothing — several past 'wins' in this family sit inside that band; (b) deep bands are much noisier than shallow ones (depth>=8 swings 3-4x primary on identical code, n=398/1 at that depth across seeds), so depth>=7/8 criteria need either far more exposure or pooled variants (depth>=8:pooled -0.0105 tracked the unpooled sign here, but depth>=4/5/6:pooled all disagreed in sign with unpooled — pooled and per-arm are not interchangeable).

Incidentally the run is itself a third instance of the pattern the audit was chartered to test: h2 up while depth>=7 and depth>=8 fell, with panelZ ~0. Under a null diff. Which is the point — that signature is producible by noise alone, so 'h-rate up, ladder down' is not evidence of a mechanism, and neither is its inverse. The decorrelation claim remains plausible but unmeasured; the honest version of it is weaker and cheaper to state: no criterion, hazard or depth, is informative until it is compared against this null band.

Process consequence: meta/offline hypotheses must not enter the same queue as add/ablate until there is an execution path that skips the campaign, or every one of them costs 10 wall-minutes to produce a needs_human stub plus a noise sample.

## 2026-08-30T08:50:29.914Z

**util-stats-export-completeness-guard** (auto_merge): Guard landed and merged (auto_merge, enabling/non-inferior; regression+lint pass, 0 violations, 2x300s VR seeds 1000/1001 at ~962/965 runs/s). Objective deltas all mildly negative and within seed noise (primary -0.0196, panelZ -0.028, throughput -0.022, h2 -0.011, depth>=8 -0.061), consistent with no behavioral change — the serialization rewrite is policy-neutral as designed. Payoff is immediate and diagnostic: steer_authority.steps_total is now visible in both the flat counters map and the mechanism dump (4808048), and it resolves the parent lineage outright. steps_total - steps = 865998392-865998268 = 124 (seed 1000) and 878533903-878533781 = 122 (seed 1001); 124 is exactly termination.all.deadlock for seed 1000. So the 'unaccounted steer steps' the provenance lineage was chasing is precisely one terminal step per deadlocked run — ~1.4e-7 of all steps, not a hidden decision path. steer-steps-provenance-ablate is thereby answered without spending an exposure campaign; it should be closed rather than run. Residual export gap: the nested summary object in the run/eval artifact (utilStats.steerAuthority) still shows only 5 hand-picked keys (steps, preferenceExpressed, preferenceHonored, honored, blockedByTimerGate) while counters carries all 14 steer_authority.* — the guard made the counters map total, but the projected summary structs consumed downstream are still hand-maintained, so a downstream reader keyed on the summary remains blind. Independently confirmed and more consequential: the steer subsystem is entirely inert under scheduler_configs/loop/general_vr.json — steer.evaluations=0, steer_terms.* all 0, and preference_consulted == preference_source_absent exactly (2597927018 and 2635519960 in the two seeds; 14424030 in the mechanism dump). Every consult finds no bound preference source, i.e. the plumbing runs 2.6e9 times per campaign to no effect. Any future steer-term hypothesis is unfalsifiable until a source is bound or the config fails loudly, and the always-taken absent branch is also a nontrivial per-step cost on the hot path.

## 2026-08-30T10:20:28.924Z

**recovery-selection-null-steer-control** (closed): Control ran to the 4-chunk cap (1,116,928 runs, seeds 1000-1003), verdict 'closed / non-inferiority unresolved'. The deliberately uninformative predicate (hash(node_id, step) parity) reproduced the parent's regression almost exactly: stratified depth>=4/5/6/7/8 = -0.029/-0.037/-0.038/-0.036/-0.028, throughput -0.040, violations +1.8e-6 (1 violation, seed 1000, grid-short arm). Parent's band was -0.04..-0.07. So the regression is NOT predicate-specific -> the entire event-side recovery-reweighting family (any predicate at this site, this magnitude) is closed, and the inverted-predicate variant is no longer worth spending on. But the term counters undercut the causal story for BOTH runs: steer_terms.recovery_weight_null_steer.evaluated=323,427 against steer_authority.steps=839,631,636 (3.9e-4 of steps), present=323,410, won=322,776, contested=2,551, flipped=0, measured=0, acted=0. The reweighting actually changed a contested outcome ~2.5k times in 8.4e8 decisions (~3e-6) and registered zero measured/acted downstream effect, which cannot mechanistically produce a -0.04 shift in depth>=6 rate. Corroborating: pooled and stratified disagree by ~1.5x at the deep end (depth>=6 pooled -0.059 vs stratified -0.038; >=8 -0.055 vs -0.028), the signature of arm-mix drift rather than a per-run effect, and the -4% throughput hit (924.8/926.3 runs/s) shifts round_robin arm run counts under a fixed wall clock. Most likely reading: adding any new steer term at this site perturbs the shared sampler/RNG draw order and the campaign's arm mix, and the resulting depth delta is an evaluation artifact, not steering. That means the parent's 'clean, large, monotone' regression is equally suspect as evidence about VR races. Actionable: stop reading single-term A/B deltas at the recovery-selection site as causal until the compiled-in-but-inert placebo below comes back flat.

## 2026-08-30T10:26:42.099Z

### Audit @5355
Per-iteration mean is ~2936 s of tracked phases; evaluate (1541 s, 52%) plus regression (~530 s, 18%) means ~70% of wall clock is spent re-measuring, and implement (618 s, 21%) is nearly all model/think (458 s of 618) with only ~14 edits and 4.1 cargo builds per iteration. propose is effectively 0 s (0.0004 s mean) because the pool is pre-stocked with 234 hypotheses, 96 of them parked. The last 15 iterations averaged ~47 min each; iteration 5353 alone burned 687 s implement + 1559 s evaluate. The real concentration problem is topical, not phase-level: of the 40 listed hypotheses, 21 are steer/steer-authority/prefix-extension instrumentation ('enabling' or 'ablate' kind) on a mechanism whose util dump shows steer.evaluations = 0 and preference_source_absent = 14424030 / 14424030 = 100%. Roughly half the recent implement budget is being spent instrumenting a code path that has never once been consulted with a bound source.

Goodhart: The objective is rung events per explore-second, a product of per-run probability and runs/s. grid-short (max_iterations 1500) posts 2176 runs/s and 85.68 depth>=6/s vs grid's 646–709 runs/s and 16.74–28.57/s — a 3–5x 'win' obtained purely by truncating runs. The arm set is being scored on how cheaply it can restart, not on exploration depth. Any allocation change that shifts share toward grid-short will look like progress while reducing distinct deep states.; Reference (run 000) meanPrefixDepth is 2.26 with P(depth>=4)=0.034; the current baseline is 3.28 with P(depth>=4)=0.466 — a 13.7x inflation on the rung that gates everything above it. Depth has saturated at ~3.28 for many iterations (prefix-extension-policy-depth-diagnostic was merged specifically to explain this) while the headline rate metrics keep moving. The metric has decoupled from the reference distribution it was meant to generalize.; h4Rate = 0.917 against a reference of 0. A hypothesis-rate metric pinned at 92% carries essentially no discriminative information and should not be counted as a coverage win; it is a predicate that fires almost always.; 42 merged hypotheses have produced no visible ladder movement: latest merged is at or below baseline on every rung (meanPrefixDepth 3.27 vs 3.28, P>=5 0.154 vs 0.160, depth>=6/s 34.48 vs 37.19, depth>=7/s 7.22 vs 7.90). Merges are being credited on 'neutral, no violation' rather than on objective gain — merge count is functioning as the gamed quantity.; Exposure asymmetry: baseline row is 1201 s (4 chunks) and the merged-candidate column is 300 s (1 chunk). Comparing a 1-chunk candidate against a 4-chunk baseline systematically favors declaring the candidate non-regressive.; A large fraction of recent work is 'enabling'/instrumentation hypotheses (counter export, serialization parity, dead-counter audits). These reliably pass because they cannot regress the objective, so they accumulate merge credit without testing any mechanism.

Utilization: steer (feedback.steer / steerAuthority)=broken, steer_empty_slice=broken, aos=unexercised, dedup=unexercised, multiplier_authority=unrewarding, curriculum=unexercised, purgatory=healthy, crash_recovery / post_fault_ops=healthy, recovery_window=healthy, feedback (timeline scoring)=unrewarding, rng_stream_isolation=scaffolding

Policy suggestions: Freeze all steer-related hypotheses. Close (not park) the ~21 steer/steerAuthority/preference/timeline-steer items as a block with a single disposition citing steer_empty_slice.candidate_mask_skipped == multiplier_authority.decisions == 4,807,934 (100% empty slice) and steer_terms all zero. Reopening requires one unit test — steer-decision-site-reachability-probe — demonstrating a nonzero slice with a bound source, at a cost of well under one 150-min hypothesis budget. This is the single largest recoverable waste: roughly half of recent implement time.; Add a hard liveness precondition to the judge: a hypothesis may not enter implement if the counter it proposes to move is currently identically zero AND no merged change in the last 20 iterations has made it nonzero. Adopt steer-mechanism-liveness-guard (gain/cost 7/2) as a grader gate rather than as another instrumented hypothesis.; Fix the exposure asymmetry: candidates must be compared at equal chunk count to the baseline (4 chunks, ~1200 s) before any merge, or the baseline must be re-cut at 1 chunk. Comparing a 300 s candidate to a 1201 s baseline biases toward 'no regression'.; Tighten the inconclusive rule: require P(better) >= 0.9 on the pre-registered primary rung (depth>=6/s) AND directional consistency across depth>=4/5/6, instead of any-rung >= 0.9. steer-preference-source-binding-guard (0.781 / 0.753 / 0.952 over 1.17M runs) fails that test and should be closed, not resumed.; Denominate the objective per-run, not per-second, or cap runs/s credit. grid-short buys 3-5x objective by truncating max_iterations to 1500; report depth>=6 per run alongside per-second and require a hypothesis to win on both before merge, otherwise the loop will keep optimizing for cheap restarts.; Reclaim the aos arm's 20% wall share (≈60 s/chunk, ~36k runs, ~24 min per day of evaluation). Either replace it with a live grid overlay or gate it behind an enabling hypothesis that makes aos.tape_wins nonzero; an arm whose defining mechanism reads zero is paying full price for grid-equivalent data.; Introduce a merge criterion with teeth: 42 merges have moved the ladder by 0 (latest merged is at or below baseline on all 7 rungs). Require either a measured objective gain at the tightened statistical bar or an explicit 'scaffolding' label that does not count toward merged totals, so merge count stops reading as progress.; Redirect the freed budget to the two mechanisms with demonstrated effect sizes: purgatory (grid-no-purgatory costs -41% depth>=6/s) and fault/recovery ordering (ordered_h3 fires 195/230 at 6 fault events vs 0/344 at 2). acceptance-window-targeted-delay-placement, purgatory-cut-holds-with-unchanged-receiver-state, within-run-distinct-crash-subject and single-crash-subject-fault-concentration are the live candidates; prioritize these over any further counter-export work.; Investigate the depth saturation at meanPrefixDepth 3.28 as a structural ceiling, not per-hypothesis noise. P(depth>=4)=0.466 but P(>=5)=0.160 and P(>=6)=0.038 implies a roughly constant ~0.24-0.34 continuation probability per level; nothing merged in 42 attempts has changed it. plan-complete-pending-frontier-continuation is the only listed hypothesis that targets the termination rule itself and should be promoted.; Cut the audit/regression overhead: regression is ~530 s (18%) of every iteration on top of a 1541 s evaluate. Given violations have been 0 across the entire visible history, run full regression every 5 iterations (matching the audit cadence) rather than every iteration, and gate it on the diff touching a lane with recorded violation history.

## 2026-08-30T10:54:03.762Z

**steer-preference-source-binding-guard** (closed): Instrumentation landed and answered the question; the throughput claim did not resolve. Both sequential seeds (1004: 288960 runs, 1005: 299744 runs, 300 s each, 14 rayon threads) report steer_authority.source_bound_sessions = 0 and source_unbound_sessions = runs exactly (288960 / 299744). Combined with preference_consulted == preference_source_absent to the unit (2.651412066e9 and 2.749994016e9) and steer.evaluations = steer_terms.* = 0, the unbound preference source under scheduler_configs/loop/general_vr.json is now a machine-readable fact in the total counters map rather than a remembered observation. Every steer hypothesis that requires preference_expressed > 0 is therefore structurally unfalsifiable on this config until the source is bound — that is the deliverable. The perf half failed to separate: sequential evaluation ran to cap (8 chunks / 2350784 runs) with non-inferiority unresolved, pGreater 0.952, throughput delta +0.0110, primary (depth>=6) +0.0070, violations 0 — exactly the near-zero regime the calibration prior predicted (steer-zero-weight-early-out 0.0000, steer-term-zero-weight-shortcircuit 0.057). Hoisting one always-false branch out of 2.6e9 consults is not measurable against 300 s campaign noise. Caveat on the artifact: regressionPassed = false on the closing decision while lint passed, so the branch is not mergeable as-is; whoever binds the source must first find which suite the added spur-core test (or the session-construction refactor) broke — the decision record does not name it. Secondary read: the hot path still pays for steer beyond the consult site — steer_empty_slice.candidate_mask_skipped 9.17e8, queue_audit_skipped 9.17e8, ranking_pass_skipped 4.04e8, timer_steer.evaluated 4.49e8 per campaign — i.e. the short-circuit was placed at the narrowest of four inert sites. Cross-seed arm behavior was stable except the aos arm (35872 vs 46464 runs, rewardRate 141.9 vs 235.1), a reminder that aos throughput variance alone can exceed the effect size being chased.

## 2026-08-30T11:14:09.657Z

**value-repr-shrink-hot-variants** (closed): Shrinking ValueKind by boxing the large cold variants (Record/env-carrying) yielded +0.6% throughput against a 5% gate — a clean falsification of the byte-count mechanism at this magnitude. Combined with the parent (unshare-for-try-unwrap, refcount topology) also failing, both available levers on the drop_glue+Value::new+ValueKind::clone cluster (~8.5% of profile) are now exhausted: the cost is not dominated by moved bytes and not by avoidable refcount traffic. The residual is almost certainly per-event call overhead and allocator traffic that boxing merely relocates (the stated falsifier's second branch), meaning the cluster is structural to the current one-Value-per-eval-node design. Corollary for the pool: micro-optimizing individual value ops is done; any further perf win must come from doing fewer value operations per simulated step (eval-loop/interning-level change) rather than making each one cheaper. Given throughput bleeds 0.8-1.8% per instrumentation merge while these fixes buy <1%, the per-second factor cannot be defended by value-layout work — the more productive direction is reducing instrumentation cost itself, where the counter dump shows enormous dead-path volume (steer_authority.preference_source_absent = 14.4M consultations with 0 expressed, steer_terms.* all-zero across 7 term families, dedup.* and steer.* fully zero, aos.* zero).

## 2026-08-30T12:05:58.122Z

**crash-anchor-eligibility-base-rate-census** (auto_merge): Census populated and is internally coherent (crash_census.decisions == crash_recovery.crashes == delivery_effects.crashes_total; victim_had_inflight_sends == crashes_with_own_sends_inflight == crash_anchor.applied), so the fault-decision site was correctly located and the numbers are trustworthy. Base rate: 180215/526937 = 0.342 (seq seed1000; 0.376 in explorer) — below the 0.4 falsifier line, so anchoring is NOT vacuous and the family is not closed on the stated criterion. But the decisive number is the one the falsifier did not anticipate: any_candidate_had_inflight_sends = 186723/526937 = 0.354, only 1.24 points above the chosen victim's 0.342. In 96.5% of decisions where ANY eligible candidate holds undelivered outbound sends, the uniform selector has already picked such a victim. Perfect victim re-ranking therefore buys at most a 1.036x relative uplift in anchored-crash frequency — victim-selection anchoring is arithmetically dead, which retro-explains the four closed attempts (-0.0625, 0.0000) without needing a bug story. The remaining headroom is entirely in WHEN to crash, not WHOM: crash_anchor.steps_with_crash_eligible = 1.78M vs 527k crashes taken means only ~30% of eligible steps become crashes, so the timing axis is unmeasured and unsaturated. Second finding, from the histogram: bucket_0 65.8%, bucket_1 9.2%, bucket_2 16.1%, bucket_3plus 8.9% — non-monotone, with exactly-one-outstanding RARER than exactly-two. Outbound sends leave a node in fan-out bursts, so 'has inflight' is close to all-or-nothing per burst and the mid-burst state (exactly one peer reached, quorum partially informed) occupies only 9% of crash decisions despite being the state that separates partial-broadcast anomalies from clean pre/post-burst crashes. That is a concrete, protocol-agnostic rare stratum to steer toward. Ladder: merged non-inferior as scaffolding (primary +0.0040, depth>=8 +0.045, depth>=7 -0.018 — all noise at +1 param, 0 violations); no merge credit claimed.

## 2026-08-30T12:53:42.386Z

**steer-decision-site-reachability-probe** (auto_merge): Probe answered the reachability question decisively; verdict auto_merge (enabling, non-inferior; primary +0.012, violations 0, no regression). Histogram is outcome (2), unanimously: steer_reach.no_weighted_predicate == steer_authority.steps == steer_empty_slice.queue_audit_skipped == 895,090,754 (seed 1000) / 893,437,808 (seed 1001) — i.e. 100.00% of steps enter the steer block and short-circuit at exactly one guard, no_weighted_predicate. The other four reach buckets (no_schedule_attempt, audit_disabled, single_candidate, ranking_agreed_with_priority) are all 0, so the early return is not contested: nothing downstream of the predicate lookup has ever executed. Consistent with that, preference_consulted == preference_source_absent == 2.685e9 (exactly 3 consults/step, all missing a source), and every terminal counter (audited, expressed, honored, blocked_by_order, blocked_by_timer_gate, no_eligible_candidates, sampler_chose_other, other_queue, steer_terms.*, steer.evaluations) is 0 across ~1.79e9 steps over both seeds. Conclusion: steerAuthority is not 'rarely honored' — it is unreachable-by-configuration under general_vr.json. The zeroes were never evidence about scheduling policy; they were evidence that the grid arm supplies no weighted predicate. Two consequences: (a) all further preference/honored instrumentation is worthless until a predicate source exists, so that lineage closes as instrumentation and reopens only as a config question; (b) the machinery still pays 3 consults + a mask/rank/audit skip per step for a guaranteed-absent lookup, ~2.7e9 wasted consults per 300s campaign at ~980 runs/s, making excision a legitimate perf candidate independent of whether the policy is ever revived. Cost of the answer: one in-process test, seconds, vs the ~half of recent implement time this family had been consuming in 300s campaigns.

## 2026-08-30T13:10:39.462Z

**steer-authority-dead-path-excise** (closed): Bypassing all four inert steer sites at scheduler construction produced -0.1% runs/s (noise), not the 1-3% hoped and far under the 5% gate. With preference_source_absent = steps = 4.81e6 and all steer_reach outcomes routed to no_weighted_predicate, the entire steer block is unreachable-by-configuration, yet excising it costs nothing measurable: the removed work (3 consults/step, mask+audit+ranking skips, 2.23e6 timer_steer evaluations) is perfectly branch-predicted and already effectively free to the CPU. This is the pre-registered falsifier, so the whole steer-excision perf lineage closes: no further hoisting/bypass/deletion variant of the steer path may be proposed. Broader calibration: schedule_runnable is 3.03% of profile, and micro-excisions inside sub-4% profile regions are below the bench noise floor -- perf work must target either the top profile entries or algorithmic step-count reduction (steps_used_sum 4.81e6 with 757/1080 runs hitting iterations_exhausted and 9014 pending events at exit), not per-step bookkeeping. Related dead surface stays dead but must not be re-probed: steer_terms.* all zero, steer.evaluations 0, steer_authority.honored 0, aos.*_wins 0, dedup.* 0, curriculum.* 0, feedback.cfg_score_sum 0.

## 2026-08-30T14:21:58.205Z

**plan-complete-pending-frontier-continuation** (closed): Falsified, and in the exact way the pre-registered falsifier named. Continuation fired on the whole target population (run_continuation.runs_extended=62,206 ≈ plan_complete=62,213, 26.9% of 231k runs) and spent the predicted slack: 101.998M extra steps (~1,640/extended run, 13.1% of all steps), pushing steps_used/step_budget from ~0.79 to 0.884. Payoff: mean prefix depth 3.34 vs ~3.26 baseline (~+0.08 depth for 1,640 steps), while throughput fell 20.3% (770 vs ~966 runs/s) and every rung fell with it (d>=4 -0.19, d>=6 -0.21, d>=8 -0.27/-0.30, h2 ratio 0.99). Depth-per-explore-second is strictly worse; the extra steps do not grade. Two structural facts explain it: (a) run_continuation.runs_drained=0 and prefix_extension.plan_complete_quiescent=0 — the pending frontier NEVER empties, so post-plan-complete pending work is self-regenerating (timer churn), not a finite tail of distinguishable events; every extended run ran to its allowance cap, so 'drain the frontier' has no stopping point. (b) timeline_keys.cumulative_distinct_keys=5 with saturation_run_index=100 and timer_effects.inert_streak.long acted_fraction=0.011 on 111.9M fired timers — the tail of a run is inert by construction. Generalization for the pool: on this rig the ladder is throughput-limited, not per-run-depth-limited; the depth deltas track the throughput delta ~1:1, so ANY policy that buys per-run depth with wall-clock loses unless it is >1.25x depth-per-run efficient. The campaign arms already say this loudly in the same log: grid-short (max_iterations=1500) delivered 466 runs/s vs grid's 142 with an equal-or-better d>=6 rate per run (0.88% vs 0.80%). The productive direction is the inverse of this hypothesis — reclaim inert tail steps, not extend them.

## 2026-08-30T15:14:18.496Z

**quiet-stretch-depth-telemetry-readonly** (auto_merge): The probe landed but never fired: quiet_stretch.runs = 0 and quiet_stretch.per_run_dropped = 0 in both sequential seeds (1000/1001) and in the mechanism census, i.e. the accumulation stayed gated off because no probe config with quiet_stretch_telemetry=true was ever run. quietStretchByDepth carries no data, so the stated success criterion (does a quiet-stretch quantile exist above which P(depth>=6) ~ 0) is unanswered. What the run did establish is (a) the gate is genuinely free — merged as non-inferior enabling with panelZ -0.098 and all depth deltas within noise, so the default hot path is untouched; and (b) the motivating family is nearly dead on arithmetic that was already on the table: prefix_extension.all.tail_without_release_sum = 16031 against steps_sum = 891,857,770 (1.8e-5 of all steps), with steps_blocked_sum = 33.2M (3.7%) and budget_blocked runs = 0. Even a perfectly-calibrated inert-tail cutoff reclaims ~0.002% of the step budget, so no threshold — separating or not — can pay for itself. Meanwhile the arm census shows the budget lever that actually works is blunt and global, not per-run adaptive: grid-short (max_iterations 1500) yields 1152 depth>=7 in 60s vs grid's 364 in 60s (19.2/s vs 6.1/s, ~3.1x) at near-identical per-run depth>=5 rates (4.26% vs 4.47%). Throughput per second, not tail trimming, is where the step budget converts to depth. Cost accounting: this is the third evaluation spent on the stall-abort/early-termination family and the first that produced a decisive (negative) argument, and it came from counters already present rather than from the new histogram.

## 2026-08-30T15:29:24.174Z

**grid-short-overlay-to-750-iterations** (closed): REJECTED and reverted. Single 300s sequential eval (370432 runs, 14 rayon threads, seed 1000, ta:08b1b45+porc:ebf06c5), arm set unchanged, only grid-short overlay 1500->750. Throughput did what it should: grid-short 215200 runs / 60.017s = 3586 runs/s vs grid@6000 649/s (5.5x). Per-run rung probability collapsed faster than that at the deep rungs: grid-short@750 d>=5 3.54%, d>=6 0.621%, d>=7 0.0869% against grid@6000 4.30% / 0.953% / 0.157% -- i.e. 0.82x / 0.65x / 0.55x, a monotone worsening ratio with rung, exactly the red-team failure mode (runs die before the crash/recovery conjunction closes). Corroborating mechanism, no new instrumentation: 291858/370432 runs (78.8%) exited iterations_exhausted, steps_used/step_budget = 853.2M/1092.8M = 78%, and recovery_window.unclosed = 85268/485009 = 17.6% of recovery windows were still open at exit -- truncation lands inside the window that produces the deep rungs. Per arm-second the shortened arm still beat the long arms (d>=6 22.3/s vs grid 6.18/s; d>=7 3.11/s vs 1.02/s), but the objective is per-run normalized, so the decision reject fired on pooled d>=4 ratio 0.99 -- and because grid-short holds 58% of all runs, its diluted per-run rates dominate the union. Two durable consequences: (a) budget-length downward from 1500 is closed -- 1500 is at or above the knee for every rung >=5; (b) structurally, ANY throughput-for-per-run-depth trade is penalized by the grading normalization regardless of events/sec, so the whole shorten-the-budget family cannot win here. Caveat on the motivating number: the recorded grid-short@1500 figure of 19.2 d>=7/s implies ~1.07% d>=7 per run, ~7x grid@6000 and ~12x grid-short@750 in this same evaluation; nothing in this run reproduces that magnitude, so it is a cross-epoch/grader artifact and should not be cited again as a within-run gradient. Unplanned but sharp observation from the arm census: aos is the deep-rung engine -- 307 of 632 pooled d>=7 events (49%) from 38208 runs (10% of runs, 636 runs/s), per-run d>=7 0.803% (5.1x grid, 9.2x grid-short@750) and d>=6 1.688%; grid-no-purgatory is the dead weight at d>=7 0.048%. Allocation, not budget length, is where the unexploited per-run depth sits. aos duplicate-tape pressure is currently mild (dedup.hits 3837/38208 checks = 10%, aos.tape_wins 3837 vs config_wins 34339), so aos has headroom before saturation.

## 2026-08-30T15:55:04.793Z

**post-fault-client-ops-probabilistic-dose** (needs_human): Knob wired correctly (post_fault_ops.pairs_skipped=77050/508258=15.2% pooled, exactly the expected mean skip of the three p-arms weighted by their 30% run share), but the response curve is FLAT, not peaked. Per-arm depth per 1k runs, d>=6: grid(p=1.0)=9.97, p25=9.99, p50=9.58, p75=9.19; d>=7: 1.53/1.40/1.36/1.49. All within Poisson noise (counts 240-392, sigma ~6%). Throughput-normalized the ordering is monotone AGAINST the hypothesis: d>=6/s = 6.53 (p=1.0) > 6.39 (p25) > 6.14 (p50) > 5.99 (p75), because rewardRate drops 184 -> 173-180/s on every p-arm while depth-per-run is unchanged. Falsifier ('no p beats integer 1 on d>=6/s') is MET: p=1.0 wins. Conclusion: there is no sub-integer optimum hiding between 0 and 1; the earlier inverted-U with dose 0 worse than 1 is not reproduced here, since p=0.25 (an effective dose of 0.25) matches full dose on depth-per-run. That implies the prior dose-0-vs-1 gap was either noise or lives entirely below effective dose 0.25 -- most likely the mechanism is near-inert on depth over its whole useful range and only costs runs/sec. Campaign verdict is unusable (arm-pool mismatch -> needs_human; pooled depth deltas -0.04 to -0.17 are pool-composition artifacts, not treatment effects), but the within-campaign arm comparison above is internally valid: all seven arms ran in the same process, same seed stream, 40-60s each. Separate, larger signal in the same table: grid-short (max_iterations=1500) is the throughput-normalized champion by a wide margin -- d>=7/s = 2.55 vs 1.00 for grid, d>=8/s = 0.050 vs 0.017 -- at only ~7% lower d>=7 per 1k runs (1.18 vs 1.53). Deep-prefix yield per second is dominated by cutting per-run iteration budget, not by tuning fault-adjacent client work. Terminations: 73% iterations_exhausted, 27% plan_complete_with_pending_work, 113 deadlocks; steps_used/step_budget = 0.766, so the budget is the binding constraint for most runs, consistent with shorter budgets buying more independent prefixes than they cost in depth.

## 2026-08-30T16:00:23.817Z

### Audit @5365
Mean per-iteration cost is ~1478 s, of which regression (523 s, 35%) and evaluate (383 s, 26%) are 61% — i.e. measurement dominates. Implement is 328 s (22%) but the last-15 breakdown shows it is almost entirely model/think (486 s of ~490 s mean; edit/read/shell ~1 s), so LLM deliberation, not code motion, is the real second-largest sink. Propose is effectively zero (0.0002 s mean; only iteration 5358 spent 303 s), meaning the loop is running almost exclusively off the 99-parked/7-proposed backlog rather than generating fresh hypotheses. Rejudge (164 s, 11%) plus reflect (69 s) add another 16% of pure meta-overhead. Recent wall clock per iteration ranges 18-52 min; the long ones (5363: evaluate 1549 s) indicate the 300 s campaign is being run ~4-5x (multi-chunk) plus a full regression, so a single hypothesis costs ~2000 s of pure compute for a 300 s-budget effect. Yield: 46 merged of 246 (19%), but 85 closed + 99 parked = 75% of the pool has consumed propose/judge cost with no metric movement.

Goodhart: Primary objective is a rate (events per explore-second), so it can be raised by making runs shorter rather than deeper: grid-short (max_iterations 1500) delivers 2154 runs/s and 91.69 depth>=6/s vs grid's 653 runs/s and 29.16/s. runsPerSec has gone 142.4 (reference) -> 949.9 (6.7x) while meanPrefixDepth only went 2.26 -> 3.32 (1.47x). Most of the objective gain is throughput, not exploration depth.; There is an open hypothesis 'grid-short-overlay-to-750-iterations' (halve iterations again) explicitly proposing more of this; it is closed, but the arm-kind category exists purely to re-weight arms, which moves the union ladder without changing any mechanism in the system under test.; h4Rate = 0.920 against a reference of 0. A hypothesis rate saturated at 92% carries almost no information and cannot discriminate between candidate changes; it should be dropped from the ladder or its predicate tightened, otherwise it silently pads any 'rates improved' narrative.; violations = 0 across the entire ladder and every arm while all h-rates and depths climb. The nominal purpose (finding violations) has zero signal; the loop optimizes proxies (prefix depth, h-rates) that have never been shown to predict violations -- and the loop's own audit hypothesis 'hazard-rate-vs-primary-decorrelation-audit' asking exactly this was closed.; P(depth>=4) is 0.482 vs reference 0.034 (14x) while P(depth>=8) is 0.001 vs 0.000. The distribution has been shifted at the cheap end of the ladder; the tail that would plausibly matter is essentially unmoved, consistent with depth-metric inflation rather than deeper state-space reach.; The 'latest merged' entry (quiet-stretch-depth-telemetry) improved h1/h2/h2b/h3 rates by ~0.004-0.007 each while depth>=6/s and depth>=7/s both fell 3.9%. Merging on the soft secondary rates while the declared primary regressed is direct metric substitution.; Nine 'enabling' hypotheses merged whose entire content is instrumentation plumbing (util-stats-export-completeness-guard, counters-piggyback-serialized-block, fault-counter-readout-plumbing, prefix-extension-policy-depth-diagnostic). These count toward the merged=46 tally and toward apparent progress without any mechanism change.

Utilization: steer / steer_authority=broken, steer_empty_slice=scaffolding, multiplier_authority=unrewarding, aos=unexercised, dedup=unexercised, curriculum=unexercised, feedback (timeline scoring)=unrewarding, purgatory=healthy, crash_recovery=healthy, recovery_window=healthy, ordered_h3=unrewarding, post_fault_ops=healthy, rng_stream_isolation=healthy

Policy suggestions: Fix or delete the steer subsystem before spending another hypothesis on it. steer_terms are all zero in the evaluation config, which mechanically guarantees no_weighted_predicate on 100% of 4.8M steps; either set a nonzero term in the baseline config (a one-line config change, not a campaign) or excise the steer path and its ~11.8M/dump counters. Impose a hard rule: no hypothesis may be judged if its buildsOn names a mechanism whose latest utilization dump is all-zero -- that rule alone would have blocked 8+ steer hypotheses and every aos/dedup hypothesis.; Stop merging on secondary rates when the declared primary regresses. quiet-stretch-depth-telemetry took depth>=6/s from 37.86 to 36.40 (-3.9%) and depth>=7/s from 7.76 to 7.46 (-3.9%) and was merged on h1/h2/h2b/h3 gains of 0.004-0.007. Require a non-negative primary point estimate, or an explicit documented override, as a merge gate.; Never compare a 300 s exposure against a 1201 s baseline. The metric ladder currently does exactly this. Require equal chunk counts (4 vs 4) for the merge comparison, or report the delta with a CI that accounts for the exposure asymmetry; at 300 s the -3.9% depth>=6 delta is under 2 sigma and should have read as inconclusive.; Drop or reallocate the aos arm. It consumes 20% of every 300 s campaign (~60 s x 4 chunks x every iteration) while tape_wins=0 and config_wins=0, and it yields 21.61 depth>=6/s against grid-short's 91.69. Reallocating that 20% to grid-short would add roughly 14 depth>=6/s to the union ladder for free -- but note that is exactly the Goodhart move, so the honest version is to reclaim the wall time as budget savings, not as a metric bump.; Decontaminate the union ladder from throughput. Report depth>=6 per-run probability alongside per-second rate, and report per-arm rungs as the primary decision surface rather than the arm-mix-weighted union. Right now grid-short contributes ~63% of union depth>=6 events purely because it runs 3.3x more runs/s at 1/4 the iteration cap; any arm-kind hypothesis can move the headline number without touching the system under test.; Retire h4 from the ladder or tighten its predicate. At h4Rate = 0.920 it is saturated and carries ~0.4 bits; it cannot discriminate between hypotheses and only inflates 'rates improved' summaries.; Reopen the decorrelation audit that was closed (hazard-rate-vs-primary-decorrelation-audit). With violations = 0 across 288512 runs x every arm x every merge, there is still no evidence that meanPrefixDepth or any h-rate predicts the thing the loop exists to find. This is the single highest-value meta-question outstanding and it costs 0.4 units by the pool's own estimate.; Cut regression cost. Regression is 523 s/iteration (35% of budget) on a loop whose merge rate is 46/246. Run full regression only on merge candidates that pass the primary gate, not on every iteration; at current merge rates that is roughly a 4-5x reduction in regression wall time.; Cap implement-phase deliberation. Last-15 implement is 486 s of model/think against ~1 s of edit/read/shell and 18.9 edits, i.e. ~26 s of thinking per edit. Set a hard implement think budget (e.g. 240 s) and force a diff-or-abandon decision at the cap.; Fix or document the ordered_h3 stratification. Runs with 1-3 fault events (534/1080, 49% of the sample) return exactly 0 h3 and 0 overlap, so half the explore budget is structurally excluded from that metric. Either gate those strata out of the h3 denominator or fix the gating condition; as written h3Rate = 0.363 is computed over a denominator that includes runs that cannot contribute.; Report recovery_window's 270/1441 (18.7%) unclosed windows as right-censored rather than folding them into mean_events_open = 8.13. The mean is biased low by roughly the censoring fraction and is being used as a depth proxy.; Restart the propose phase. Mean propose time is 0.0002 s and 99 hypotheses sit parked; the loop is strip-mining a stale backlog assembled under assumptions (live steer, live aos) that the utilization dumps have since falsified. Requeue-and-revalidate the parked pool against current utilization before drawing from it again.

## 2026-08-30T16:27:41.435Z

**post-fault-client-ops-zero-ablation** (closed): Setting post_fault_client_ops = 0 in the graded scalar settings is a null on depth and a small loss on throughput. Across the two sequential seeds the base arms did go to zero ops (edges_added tracks pairs_seen only via grid-post-fault-2's ops=2 overlay; pairs_skipped stayed 0, so no gating branch fired), while depthAtLeast per 1k runs moved within the A/A band at every rung: primary d>=6 delta -0.028, d>=4 -0.018, d>=5 -0.108, d>=7 -0.050, d>=8 -0.179 (all noise-scale, all negative-leaning), and throughput fell -0.034. Verdict closed as 'non-inferior but no cost improvement': the mechanism carries no depth signal anywhere on its range 0..2, but removing it did not buy runs/s either, so the secondary falsifier fired and the only remaining payoff for the deletion is the -2 tunable count, not speed. Notably the in-campaign positive control grid-post-fault-2 (ops=2) still produced the highest d>=7/d>=8 tail counts of any grid arm in both seeds (75 and 59 at d>=8 vs 49/54 for plain grid), which is the one whisper of a real effect left — but it is confounded with that arm's higher runs and did not survive at the pooled level. Family conclusion, per its own pre-registered stopping rule: post_fault_client_ops is closed permanently. Five evaluations spent (dose grid, quiescence window, probabilistic dose, in-campaign arm, zero ablation) and the entire family has produced no depth movement outside noise and no throughput movement in either direction. The generalisable lesson for the campaign: a knob that is flat from 0 through its full range across five independent parameterisations is dead, and the cheap way to reach that verdict earlier is to run the zero point FIRST rather than fifth — dose-response ladders that begin at a nonzero point cannot distinguish 'weak effect' from 'no effect' and burn evaluations proving flatness in the interior. Also confirmed operationally: in-place scalar overrides preserve the arm mix and are the correct encoding for ablations, whereas expressing a dose as new campaign arms would have been unscoreable.

## 2026-08-30T17:19:24.417Z

**recovery-steer-identity-multiplier-placebo** (auto_merge): FALSIFIER FIRED (partially). The identity-clamped placebo is provably inert — recovery_weight_placebo.flipped == 0 across both seeds (evaluated 199560/206701, present == won == evaluated, contested 2638/2778 of 8.4e8/8.76e8 decisions) — so no candidate rank ever changed, yet depth still moved negative in the parent's direction: primary (depth>=6) -0.0128, depth>=4 -0.0100/-0.0146 pooled, depth>=8 -0.0267 stratified / -0.0545 pooled, throughput -0.0105. Verdict auto_merge only because the flag-off stage is non-inferior; the flag-on arm reproduces ~1/3 of the parent null-steer band (-0.02..-0.06) with zero causal mechanism. Interpretation: at this site a pure code-path/throughput cost of ~1% buys a depth deficit of order -0.01 (shallow) to -0.05 (deep, pooled). The previously assumed null band |primary| <= 0.003 is wrong by ~4-15x. Corroborating artifact signature in the raw arm data: under round_robin's fixed 60s-per-arm wall clock the arm mix itself drifts seed-to-seed independent of any diff — aos runs 35392 (seed 1000) vs 40160 (seed 1001), +13.5%, with materially different depth profiles (aos depth>=6 279/35392 = 7.9e-3 vs 320/40160 = 8.0e-3 but depth>=7 42 vs 39), and global runs/s 932.8 vs 947.0 (+1.5%) — i.e. the seed-to-seed throughput spread alone is larger than the measured throughput delta being attributed to the diff. Consequence: every single-term event-side A/B at the recovery-selection site whose |primary| < ~0.03 is uninterpretable, including the 'clean monotone regression' from recovery-anchored-to-own-inflight-sends and the -0.0377 null-steer control. It does NOT follow that the site is provably insensitive — the placebo delta is smaller than the parent's, so a real steering component of order -0.02 may still exist underneath; the two are not separable with this harness. Also note this run stayed at violations=0 with h2 +0.0018 and the whole steer_terms.* family at zero (no weighted predicate: steer_reach.no_weighted_predicate == steps), so nothing else was competing for draws. Action: no further event-side A/B at this site until the allocator is decoupled from wall clock and a self-vs-self null band is measured per depth bin; the placebo bool is removed with this experiment.

## 2026-08-30T18:00:00.000Z

**STANDING RULE - zero point first** (binding at judge time; full text and the off-point registry in research/observations/ZERO_POINT_FIRST.md): a hypothesis that doses or tunes a scalar scheduler knob may not open a SECOND evaluation in that knob's family until the knob-off point has been measured and recorded in that registry. Score expectedGain 0 and name this rule for any dose, sweep, response-curve, window or schedule proposal on a knob whose registry status is `outstanding`; the admissible replacement is the off point itself. A family whose graded value is already the off value (steer_terms.* at 0) satisfies the rule, and its first evaluation is a nonzero point. The off measurement has a required shape: (a) an in-place scalar override in the graded settings of scheduler_configs/loop/general_vr.json with the arm list unchanged - never a new or re-overlaid campaign arm, since a changed arm mix moves the rungs by itself and makes the contrast unscoreable; (b) the mechanism's own firing counters named in the hypothesis `firingCounter` field and reported per arm, reading zero in every arm without a deliberate positive-control overlay, so "the mechanism was off" is checked rather than asserted; (c) primary rung and throughput read under the usual protocol. Definitions: scalar knob = a numeric config field scaling how often/hard/long a mechanism acts; booleans, arm/allocation/reward structure and run budgets (max_iterations is not a dose and has no off value) are out of scope; two fields are one family when setting either to its off value silences the same counter. Scope is evaluation ORDER within a family and nothing else - not the gate, the sequential protocol, the runner, the fidelities, which config anything is graded against, or whether a flat knob is later deleted. Basis: post_fault_client_ops spent five evaluations (dose grid, quiescence window, probabilistic dose, in-campaign arm, zero ablation) to reach a verdict the zero point alone delivers first, because interior dose points cannot separate 'weak effect' from 'no effect'. Registry as of this entry: post_fault_client_ops off point MEASURED (null on depth, throughput -0.034, family closed); OUTSTANDING - purgatory.delay_probability / purgatory.delay_duration_range (counter purgatory.delayed_sends; the grid-no-purgatory arm overlay at 0.0 is a different arm mix and does NOT discharge it) and partial_fanout_crash_bias (counters crash_anchor.timing_bias_examined / timing_bias_withheld); quick_fire_multiplier is blocked on reachability (steer_reach.no_weighted_predicate on every step) rather than on this rule. This entry is a proposer/judge rule only: no spur, config or harness code changed.
